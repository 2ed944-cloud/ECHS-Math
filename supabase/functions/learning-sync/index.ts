...
Deno.serve(async req=>{
  ...
  try{
    ...
    const attempts=(payload.attempts??[]).slice(-10000),sessions=(payload.sessions??[]).slice(-1000),review=(payload.review??[]).slice(-5000);
    ...
    return reply(req,{ok:true,deprecated:true,authoritative_mastery_service:"mastery-evidence",client_mastery_ignored:Array.isArray(payload.mastery)&&payload.mastery.length>0,synced:{attempts:attemptRows.length,sessions:sessionRows.length,review:reviewRows.length,assignment_results:assignmentRows.length}});
