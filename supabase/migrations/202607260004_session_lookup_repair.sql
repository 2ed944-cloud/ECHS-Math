...
begin
  ...
  if v_session_id is null then
    raise exception using
      errcode = 'P0001',
      message = 'Session self-test did not create a session';
  end if;
  ...
  if v_lookup_account_id is distinct from v_account_id then
    raise exception using
      errcode = 'P0001',
      message = 'Session self-test did not resolve the expected account';
  end if;
  ...
  if exists (
    select 1
    from public.api_session_lookup(v_token_hash) as revoked_lookup
  ) then
    raise exception using
      errcode = 'P0001',
      message = 'Session self-test resolved a revoked session';
  end if;
