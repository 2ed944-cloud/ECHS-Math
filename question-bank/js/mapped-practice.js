...
const modeCopy={
manual:["Focused practice","Build a set from the exact course, lesson or unit selected below."],
adaptive:["Adaptive practice","The next question responds to mastery evidence while remaining inside the selected scope."],
review:["Spaced review","Only due review questions inside the selected course and scope are included."],
mistakes:["Mistake recovery","Only unresolved questions inside the selected course and scope return."]
};
const scopeCopy={
course:["Full course","Browse every mapped question in the selected course. Available to teachers and administrators."],
unit:["Completed unit","Practise all mapped questions from one unit. Students unlock it after completing every available lesson in that unit."],
lesson:["Completed lesson","Practise only questions mapped to one completed lesson."],
};
const student=()=>access?.role==="student";
const staff=()=>["teacher","admin"].includes(access?.role||"");
const normaliseCourse=value=>ECHSPortalAccess.normaliseCourseKey(value||"");
const courseDefinitions=()=>Array.isArray(window.ECHS_COURSES)?window.ECHS_COURSES:[];
...
try{
const value=JSON.parse(localStorage.getItem("echs_math_complete")||"[]");
...
return{raw:String(value),course:normaliseCourse(parts[0]),unit:Number(parts[1])+1,topic:String(parts[2]),title:parts.slice(3).join("::")};
}
function courseDefinition(courseKey){
return courseDefinitions().find(course=>normaliseCourse(course?.id||course?.course||course?.title)===normaliseCourse(courseKey))||null;
}
...
return lessons.filter(lesson=>Boolean(lesson?.url)&&lesson?.kind!=="assessment"&&!/assessment|review/i.test(String(lesson?.kind||"")));
}
function lessonCompleted(courseKey,unitNumber,topic){
const normal=normaliseCourse(courseKey);
return completionRows().map(completionParts).filter(Boolean).some(row=>
row.course===normal&&String(row.unit)===String(unitNumber)&&String(row.topic)===String(topic)
);
}
function unitCompleted(courseKey,unitNumber){
const lessons=readyLessons(courseKey,unitNumber);
return lessons.length>0&&lessons.every(lesson=>lessonCompleted(courseKey,unitNumber,lesson.number));
}
function completedUnits(courseKey){
const count=courseDefinition(courseKey)?.units?.length||0;
return Array.from({length:count},(_,index)=>index+1).filter(unit=>unitCompleted(courseKey,unit));
}
function completedLessons(courseKey){
const course=courseDefinition(courseKey);
if(!course)return[];
return(course.units||[]).flatMap((unit,unitIndex)=>
(unit.lessons||[]).filter(lesson=>lesson?.url).map(lesson=>({
unit:unitIndex+1,
...
let chosen=preferred||params.get("scope")||(requestedTopic?"lesson":requestedUnit?"unit":staff()?"course":"lesson");
if(!options.some(option=>option.value===chosen))chosen=options[0]?.value||"lesson";
UI.scope.innerHTML=options.map(option=>`<option value="${option.value}">${ECHSBank.escape(option.label)}</option>`).join("");
...
const unit=Number(requestedUnit||1),selected=ECHSBank.selectedBundleFromParams(catalog)?.row;
const source=sourceForLesson(courseKey,unit,requestedTopic)||
(selected&&normaliseCourse(selected.course_key||courseKey)===normaliseCourse(courseKey)?{...selected,course_key:courseKey,unit,topic:requestedTopic}:null);
if(source)out.unshift({
id:`lesson:${courseKey}:${unit}:${requestedTopic}`,
label:params.get("title")||`Lesson ${requestedTopic}`,
course:courseKey,scope:"lesson",unit,topic:String(requestedTopic),source,
locked:student()&&!(fromLesson?ECHSPortalAccess.lessonCompleted(fromLesson):lessonCompleted(courseKey,unit,requestedTopic))
});
}
}
targets=out;
...
let chosen=preferredId||params.get("target")||params.get("bundle")||"";
if(requestedTopic){
const direct=rows.find(row=>String(row.topic)===String(requestedTopic)&&String(row.unit)===String(requestedUnit||row.unit));
if(direct)chosen=direct.id;
}else if(requestedUnit&&UI.scope.value==="unit"){
const direct=rows.find(row=>String(row.unit)===String(requestedUnit));
if(direct)chosen=direct.id;
}
if(!rows.some(row=>row.id===chosen))chosen=rows[0]?.id||"";
UI.bundle.innerHTML=rows.length?rows.map(row=>`<option value="${ECHSBank.escape(row.id)}">${ECHSBank.escape(row.label)}${row.locked?" · Locked":""}</option>`).join(""):'<option value="">No unlocked targets yet</option>';
...
function targetLocked(){return Boolean(student()&&!assignmentId&&currentTarget()?.locked);}
...
if(total>1)UI.status.innerHTML=`<span class="pill">Loading mapped practice ${completed} of ${total}…</span>`;
});
...
UI.section.innerHTML='<option value="all">All sections</option>'+[...sections].sort((a,b)=>a[0].localeCompare(b[0],undefined,{numeric:true})).map(([value,label])=>`<option value="${ECHSBank.escape(value)}">${ECHSBank.escape(ECHSBank.cleanStudentLabel(label))}</option>`).join("");
...
setBusy(true);
UI.status.innerHTML='<span class="pill">Loading exact mapped practice…</span>';
...
if(detail.requestKey&&String(detail.requestKey)!==String(currentSourceKey))return;
if(normaliseCourse(detail.course||"")!==normaliseCourse(currentCourse()))return;
if(!Array.isArray(detail.questions))return;
loaded=detail.questions;
strictScopeInPlace(loaded,currentTarget());
updateInventory({statusText:detail.complete
...
if(mode==="review"){
const ids=new Set(ECHSLearning.dueReviews({course:currentCourse(),unit:currentTarget()?.scope==="course"?null:currentTarget()?.unit}).map(row=>String(row.questionId)));
rows=rows.filter(question=>ids.has(String(question.id)));
}
if(mode==="mistakes"){
const ids=new Set(ECHSLearning.mistakes({course:currentCourse(),unit:currentTarget()?.scope==="course"?null:currentTarget()?.unit}).map(row=>String(row.questionId)));
rows=rows.filter(question=>ids.has(String(question.id)));
}
return rows;
}
function pathwayBanner(){
const target=document.getElementById("pathwayBanner"),selected=currentTarget();
if(!selected){target.innerHTML='<div class="lockedPractice"><h2>No practice is unlocked yet</h2><p>Complete a lesson, then return here to practise its mapped questions.</p><a class="button wine" href="../index.html#courses">Return to learning path</a></div>';return;}
if(targetLocked()){
const message=selected.scope==="unit"?"Complete every available lesson in this unit to unlock full-unit practice.":"Complete this lesson before opening its practice.";
...
UI.start.disabled=true;return;
}
const roleText=student()?"Unlocked from your completed learning pathway":"Full staff visibility across mapped course content";
...
target.innerHTML=assignmentId?`<div class="notice"><strong>Teacher assignment:</strong> ${ECHSBank.escape(assignmentTitle||assignmentId)}. Results will synchronise with your learning record.</div>`:"";
}
...
if(!session)return;
const target=currentTarget();
ECHSLearning.patchSession(session.id,{questionIds:set.map(question=>question.id),answered:state.graded,correct:state.correct,index:state.index});
ECHSLearning.setContinue({
type:"practice",label:`${modeCopy[UI.mode.value]?.[0]||"Practice"} · ${target?.label||courseLabel(currentCourse())}`,
url:sessionURL(),targetId:target?.id,course:currentCourse(),scope:target?.scope,mode:UI.mode.value,
questionIds:set.map(question=>question.id),index:state.index,targetCount,correct:state.correct,graded:state.graded,
answeredIds:[...state.answered],sessionId:session.id,assignmentId
});
}
function render(){
const question=set[state.index];
if(!question){
UI.shell.innerHTML='<div class="empty"><div class="emptyState"><div class="emptyStateIcon">!</div><h2>No matching questions</h2><p>Change the question type or ask your teacher to review the lesson mapping.</p></div></div>';
return;
}
...
if(["mcq","true_false"].includes(question.type))document.querySelectorAll(".choice").forEach(button=>button.onclick=()=>{
if(state.checked)return;
document.querySelectorAll(".choice").forEach(item=>item.classList.remove("selected"));
button.classList.add("selected");state.response=button.dataset.id;
});
...
const target=currentTarget();
ECHSBank.saveAttempt(question,correct,state.response,{mode:UI.mode.value,sessionId:session?.id,assignmentId,durationMs:Date.now()-questionStartedAt,course:currentCourse(),unit:target?.unit,topic:target?.topic});
}
if(["mcq","true_false"].includes(question.type))document.querySelectorAll(".choice").forEach(button=>{
if((question.correct_choice_ids||[]).includes(button.dataset.id))button.classList.add("correct");
else if(button.dataset.id===state.response)button.classList.add("incorrect");
button.disabled=true;
});
const feedback=document.getElementById("feedback");
feedback.className=`feedback show ${correct===false?"incorrect":"correct"}`;
const accepted=(question.accepted_answers||[]).join(" / "),adaptiveNote=UI.mode.value==="adaptive"&&auto?`<p><b>Next step:</b> ${correct?"difficulty may increase while staying in this scope":"the next question will reinforce this exact lesson or unit"}.</p>`:"";
...
const candidates=ECHSLearning.selectAdaptive(eligibleRows(),1,{excludedIds:set.map(question=>question.id),lastCorrect:lastResult});
if(candidates.length){set.push(candidates[0]);state.index++;render();scrollQuestionIntoView();return;}
}
finish();
}
function finish(){
const percentage=state.graded?Math.round(state.correct/state.graded*100):null,target=currentTarget();
if(session)ECHSLearning.endSession(session.id,{questionIds:set.map(question=>question.id),answered:state.graded,correct:state.correct,score:percentage,assignmentId,course:currentCourse(),unit:target?.unit,topic:target?.topic});
...
if(!rows.length){
UI.shell.innerHTML='<div class="empty"><div class="emptyState"><div class="emptyStateIcon">0</div><h2>No questions match these choices</h2><p>Try all question types, all difficulties, or ask an administrator to check the lesson mapping.</p></div></div>';
return;
}
targetCount=Math.min(rows.length,Number(UI.count.value)||10);lastResult=null;
...
const target=currentTarget();
session=ECHSLearning.startSession({type:"practice",mode:UI.mode.value,course:currentCourse(),unit:target?.unit,topic:target?.topic,scope:target?.scope,targetId:target?.id,bundleLabel:target?.label,questionIds:set.map(question=>question.id),targetCount,assignmentId,title:assignmentTitle});
...
if(!saved||saved.type!=="practice"||saved.targetId!==currentTarget()?.id)return false;
UI.mode.value=saved.mode||"manual";updateModeCopy();
targetCount=Number(saved.targetCount)||saved.questionIds?.length||10;
const map=new Map(loaded.map(question=>[String(question.id),question]));
set=(saved.questionIds||[]).map(id=>map.get(String(id))).filter(Boolean);
...
UI.shell.innerHTML='<div class="lockedPractice"><h2>This course is not assigned to your account</h2><p>Return to your dashboard or ask your teacher to update the class roster.</p></div>';
return;
}
if(requested&&courseKeys().includes(requested))UI.course.value=requested;
if(UI.visibility){
UI.visibility.value=staff()&&params.get("visibility")!=="ready"?"all":"ready";
