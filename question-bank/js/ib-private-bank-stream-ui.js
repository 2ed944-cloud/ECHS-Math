/* Refresh the focused-practice inventory while the authenticated IB bank streams in. */
(function(){
  "use strict";
  ...
  function refresh(event){
    ...
    questions.forEach(question=>{const value=String(question?.source?.section||"unmapped"),title=question?.source?.section_title||question?.source?.skill_title||"";sections.set(value,value==="unmapped"?"General practice":`${value}${title?` · ${title}`:""}`)});
    ...
    const blockedText=blocked?` · ${number(blocked)} non-catalog mappings withheld`:"";
    if(detail.partialError)status.innerHTML=`<span class="pill gold">${number(loaded)} questions ready</span><span class="pill">Background loading will retry after refresh</span>`;
    else if(complete)status.innerHTML=`<span class="pill teal">${number(loaded)} questions available</span>${blockedText?`<span class="pill gold">${escape(blockedText.slice(3))}</span>`:""}`;
    else status.innerHTML=`<span class="pill teal">${number(loaded)} questions ready now</span><span class="pill">Loading ${number(total)} mapped questions in the background…</span>`;
  }
