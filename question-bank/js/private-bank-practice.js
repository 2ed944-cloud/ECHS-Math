...
(function(){
  ...
  async function ibPackageQuestionCount(){
    try{
      const res=await ECHSInstitution.api("private-bank-api","/packages");
      const rows=(res?.packages||res?.rows||res||[]);
      const list=Array.isArray(rows)?rows:[];
      return list.filter(r=>{
        const t=(r?.manifest?.target_courses||r?.target_courses||r?.course_keys||[]);
        return (Array.isArray(t)?t:[]).map(String).includes("ib-math-ai")||String(r?.bank_code||"").startsWith("IBAI-");
      }).reduce((sum,r)=>sum+Number(r.question_count??r.questions??r?.manifest?.questions??r?.indexed?.questions??0),0);
    }catch(_e){return null}
  }
  async function explainEmptyIB(source,error){
    const shell=document.getElementById("shell");
    const access=await resolvedAccess();
    let title="Uploaded IB banks are not showing yet";
    let body="";
    if(error){
      body="The private question service could not be reached right now. Please check the institution connection and try again.";
    }else if(!requestedLesson&&!staff(access)){
      title="Open IB practice from a lesson";
      body="Full-course browsing of IB banks is available to teachers and administrators. Students open IB practice from an unlocked lesson on the Learning Path.";
    }else{
      const uploaded=await ibPackageQuestionCount();
      if(uploaded&&uploaded>0){
        title="IB banks uploaded — pending release for practice";
        body=`Your IB packages are uploaded (${uploaded.toLocaleString()} questions indexed), but none are released for student practice yet. Release them from the Question Trust Center / re-run the secure import with the student-ready contract so each question carries verified course, lesson and skill mappings.`;
      }else if(uploaded===0){
        title="No IB questions found for this scope";
        body="The connection is working, but no IB questions match this course/lesson scope. Confirm the package target course is IB Mathematics AI and that lesson mappings were included.";
      }else{
        body="The private question service is reachable but returned no questions for this scope.";
      }
    }
    if(shell)shell.innerHTML=`<div class="empty"><div class="emptyState"><div class="emptyStateIcon">i</div><h2>${title}</h2><p>${body}</p></div></div>`;
    document.documentElement.dataset.ibEmptyReason=error?"connection":(!requestedLesson&&!staff(access))?"lesson-scope":"pending-release";
  }
  ...
  function install(){
    ...
    ECHSBank.loadBundle=async source=>{
      ...
      // Curated ECHS skill bundles ship in the repository and load without a backend release.
      // Any released private rows are merged in on top when available.
      if(source&&source.echs_curated_skill){
        const [base,direct]=await Promise.all([
          originalLoad(source),
          privateRows(source).catch(()=>[])
        ]);
      ...
      if(activeCourse==="ib-math-ai"){
        try{
          ...
          updateIBCourseOption(source,direct.length,direct.length?"ready":"empty");
          ...
          if(!direct.length)await explainEmptyIB(source);
          ...
          await explainEmptyIB(source,error);
          /* AP Calculus fallback banks were intentionally blocked for IB Mathematics AI. */
