...
(function(){
  ...
  function normalise(row){
    ...
    const visible=aliases.student||aliases[course]||aliases.teacher||labels[course]?.[question.bank_code]||question.bank_code;
    ...
    question.classification={
      ...
      ib_unit:course==="ib-math-ai"?mapping.unit:question.classification?.ib_unit,
      ib_lesson:course==="ib-math-ai"?lessonKey:question.classification?.ib_lesson,
      ib_lesson_title:course==="ib-math-ai"?lessonTitle:question.classification?.ib_lesson_title,
      // Legacy compatibility fields are retained because the existing focused-practice client reads them.
  ...
  async function privateRows(){
    ...
    const seen=new Set();
    return rows.map(normalise).filter(question=>{
  ...
  function install(){
    ...
    ECHSBank.bankLabel=code=>dynamicLabels.get(code)||labels[course]?.[code]||originalLabel(code);
      ...
      if(result&&question?._private_bank){
        ...
        if(index>=0){
          rows[index]={
            ...
            trust_tier:question.trust_tier||"publisher_key_direct",
            representation:"publisher-source",
            verification_basis:question.metadata?.verification_basis||"publisher-answer-key"
          };
    ...
    document.documentElement.dataset.privateLessonKey=lesson;
  }
  install();
})();