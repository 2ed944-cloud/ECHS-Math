(()=>{
  ...
  function render(){
    ...
    $("accountRows").innerHTML=rows.length?rows.map(row=>`<tr>
      ...
      <td>${esc(date(row.last_login_at))}</td><td><div class="tableActions">
        <button class="iButton secondary small" data-reset="${esc(row.id)}">Reset password</button>
        ${row.id!==current.id?`<button class="iButton ${row.status==="active"?"danger":"teal"} small" data-status="${esc(row.id)}" data-next="${row.status==="active"?"suspended":"active"}">${row.status==="active"?"Suspend":"Activate"}</button>`:""}
    ...
    document.querySelectorAll("[data-reset]").forEach(button=>button.addEventListener("click",()=>{
      const row=accounts.find(item=>item.id===button.dataset.reset);$("passwordAccountId").value=row.id;$("passwordTarget").textContent=`Set a new school-managed password for ${row.display_name} (@${row.username}).`;$("newPassword").value="";$("resetCredential").classList.add("hidden");open($("passwordDialog"));
    }));
  ...
  async function init(){
    current=await ECHSInstitution.requireAuth(["admin"]);if(!current)return;
    ECHSInstitution.mountIdentity(current);await load();
    ["accountSearch","roleFilter","statusFilter"].forEach(id=>$(id).addEventListener(id==="accountSearch"?"input":"change",render));
    ["openCreate","heroCreate"].forEach(id=>$(id).addEventListener("click",()=>{$("createForm").reset();$("createdCredential").classList.add("hidden");open($("createDialog"));}));
    ["openImport","heroImport"].forEach(id=>$(id).addEventListener("click",()=>{csvRows=[];$("importPreview").innerHTML="";$("runImport").disabled=true;$("csvFile").value="";open($("importDialog"));}));
    $("downloadTemplate").addEventListener("click",()=>download("echs-account-import-template.csv","display_name,email,username,password,role,grade,external_id,class_name\nAmina Hassan,amina@example.edu,,,student,11,S001,AP Precalculus\nMr Ahmed,ahmed@example.edu,ahmed.teacher,,teacher,,,"));
    ...
    $("createForm").addEventListener("submit",async event=>{
      event.preventDefault();const form=new FormData(event.target),payload=Object.fromEntries(form.entries());payload.can_manage_accounts=$("createManage").checked;
    ...
    $("importForm").addEventListener("submit",async event=>{
      event.preventDefault();$("runImport").disabled=true;$("runImport").textContent="Importing…";
    ...
    $("passwordForm").addEventListener("submit",async event=>{
      event.preventDefault();const response=await ECHSInstitution.api("account-api",`/accounts/${$("passwordAccountId").value}/reset-password`,{method:"POST",body:JSON.stringify({password:$("newPassword").value})});
