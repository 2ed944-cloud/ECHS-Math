...
function walkStrings(value, field = "") {
  ...
  if (Array.isArray(value)) {
    return value.flatMap((item, index) => walkStrings(item, `${field}[${index}]`));
  }
...
for (const fileName of overlayFiles) {
  ...
  for (const [index, record] of payload.records.entries()) {
    ...
    if (seen.has(id)) {
      errors.push({ file: fileName, id, issue: `Duplicate overlay ID; first seen in ${seen.get(id)}.` });
    } else {
      seen.set(id, fileName);
    }
    if (canonicalIds.size && !canonicalIds.has(id)) {
      errors.push({ file: fileName, id, issue: "Overlay ID is not present in canonical/admin question chunks." });
    }
    ...
    if (labels.length !== new Set(labels).size) {
      errors.push({ file: fileName, id, issue: "Duplicate part labels within question." });
    }
...
fs.writeFileSync(
  path.join(reportDir, "ADMIN_OVERLAY_VALIDATION.json"),
...
const report = [
  ...
  "| Measure | Result |",
  "| --- | ---: |",
  `| Overlay files checked | ${overlayFiles.length} |`,
  `| Overlay records checked | ${recordCount} |`,
  `| Unique overlay IDs | ${seen.size} |`,
  `| Canonical IDs loaded | ${canonicalIds.size} |`,
  `| Math-bearing fields checked | ${mathFieldCount} |`,
  `| KaTeX expressions parsed | ${expressionCount} |`,
  `| Errors | ${errors.length} |`,
  "",
  ...(errors.length
    ? ["## Errors", "", ...errors.slice(0, 300).map((item) => `- \`${item.file}\` ${item.id ? `\`${item.id}\` ` : ""}${item.field ? `\`${item.field}\` ` : ""}${item.issue}`), ""]
    : ["Zero JSON, duplicate-ID, access-gate, delimiter, or KaTeX parser errors remain.", ""]),
