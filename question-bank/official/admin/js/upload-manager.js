(function(){
  ...
  const drop=$("dropZone"),fileInput=$("zipFile"),summary=$("fileSummary"),nameNode=$("fileName"),metaNode=$("fileMeta"),privateBankFields=$("privateBankFields"),releaseFields=$("releaseFields"),start=$("startUpload"),clear=$("clearUpload"),bar=$("progressBar"),percent=$("progressPercent"),stage=$("progressStage"),history=$("requestList");
  ...
  function selectKind(kind){
    ...
    $("managerCopy").textContent=kind==="private-bank"?"Choose one ECHS private-bank ZIP. AP Calculus packages are detected from the manifest, validated, imported and connected to private practice automatically.":"Choose a reviewed unit ZIP. The processor will validate the package, create a repository branch and open a pull request automatically.";
    resetFile();
  }
  ...
  async function begin(){
    ...
    try{
      ...
      if(created.duplicate){setProgress(created.request.progress,created.request.stage||"This package already exists");watch(created.request.id);await loadHistory();return}
      if(!created.upload?.signed_url)throw new Error("Secure upload URL was not returned");
      setProgress(12,"Uploading ZIP to private staging");await signedUpload(created.upload.signed_url,state.file);setProgress(82,"Confirming upload");
