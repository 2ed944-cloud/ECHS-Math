...
PR #356 deployed the implementation and migration after 19 passing PR workflows. Actual PostgreSQL 15.19 passed 235 original and 203 new checks, plus eight original and seven new HTTP/SQL groups. All 16 main/live workflows and 31 public checks passed. Subsequent inspection found that the production RPC transport omitted `lesson_content_capabilities`, leaving the new controls disabled. The corrective slice permits exactly that RPC, tests the real handler through the actual transport, and adds a data-free `/health/authoring` probe for deployed-path verification. Completion remains pending the repair's merge and deployment checks.
...
Unresolved release risks: the capability transport repair must pass CI and production verification before completion; synthetic browser tests cannot replace teacher acceptance with a real school account. No production class pin, teacher draft creation, review or publication is performed by this validation stage. The new health probe exposes only the fixed public capability contract and uses the same service transport/database probe as authenticated context; it returns an unavailable response when the contract cannot be verified. Existing `/health` and authentication requirements for all private context remain unchanged.
