...
- ECHS-008: implemented and locally verified; secure Studio shell, class/catalog selection, immutable slide operations, recoverable deletion, private preview and serialized autosave. All 14 model, 20 session, 17 transport, 16 browser groups and 68 baseline checks passed. No database migration, publication change, student route cutover or private browser persistence. Exact PR/deployment checks are pending; ECHS-009 implementation has not started. See `LESSON_STUDIO_FOUNDATION.md`.
