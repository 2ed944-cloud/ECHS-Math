...
Teacher attention details and student-row meters distinguish missing evidence from a recorded zero. Missing evidence has explicit text and an empty fill; invalid `null%` text or CSS cannot produce an apparent full meter. Existing alert selection and numerical aggregates remain unchanged.

The existing metric animator also treated its own first frame as an external update and stopped early, leaving a value such as 9% for a target of 97%. Its repair preserves the target text, honors reduced motion and cancels stale animation writes when an external value or account changes. This is a display correction; no stored score or mastery calculation changes.
...
The affected local suite contains 20 commands and 18 actual Chromium groups, including both old/new workers, synthetic accounts, desktop/phone views, unavailable evidence, zero/missing distinctions, responsive widths and preserved entry behavior. It includes nine regressions against the actual metric animator. There are 20 captured screenshots with representative views reviewed. Twelve new worker groups and all 26 existing C01 worker groups must pass. Independent review tested 315 forged/missing/zero combinations and eight numeric sequences; it found no open composition defect. Final evidence binds the complete rerun and visual review to the repaired source; local results are not a production claim.
...
The broader platform capture uses locator waits under the unchanged production CSP and real-shaped synthetic class/API fixtures. It checks the provisional heatmap's exact recorded scores, seven missing cells, coverage and lack of certification, then verifies that an unavailable response clears previous scores and that recovery restores them. Partial route captures are local diagnostics only; CI must retain its complete route set. Expected synthetic 503 console messages are bounded to their exact route/status/count; other errors still fail.
