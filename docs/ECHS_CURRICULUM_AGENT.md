# ECHS Grade 9 + Grade 10 Path B Production Agent

## Mission

Bring Grade 9 and the Grade 10 AP Readiness / Transition Pathway (Path B) to verified instructional readiness without hiding gaps behind course cards, generic routes, older lesson files, or inflated counts. The final share-ready 2026-2027 Grade 9 and Path B documents are authoritative for this production scope.
...
- Grade 10 Path B: 6 units and 51 teachable blocks (4, 10, 11, 13, 7, 6), plus the R1.G and R2.G transfer-gateway assessment packages.
...
1. Read current `main`, issue `ECHS curriculum completion queue`, and open agent-authored Grade 9/Path B production PRs.
2. Run `python tools/echs_curriculum_agent.py`; repair any structural error or false `ready` claim first.
3. Select exactly one unfinished module in deterministic order: Grade 9 sequentially first, then Path B sequentially. Continue an open agent-authored PR for that module instead of creating a duplicate.
4. Build the module on a clean short-lived branch. Match the proven AP Precalculus/AP Calculus architecture and visual standard while respecting the source-defined Grade 9 or Path B content ceiling. Never mass-copy identical content with renamed labels.
...
- all 51 Path B blocks and both transfer-gateway packages are `ready`;
...
- the final Grade 9/Path B sequencing, mathematics, answer-key, route, mobile, accessibility, and deployment audit passes.
