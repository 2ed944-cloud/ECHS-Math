...
def source_receipt(repo):
    ...
    for item in pins["files"]:
        path = item["path"]
        need(type(path) is str and not Path(path).is_absolute() and ".." not in Path(path).parts and "\\" not in path, "source-path")
        base = HERE if item["scope"] == "candidate" else repo
        data = (base / path).read_bytes()
        need(len(data) == item["bytes"] and sha(data) == item["sha256"], "source-changed")
        records.append(item)
    migrations = [x for x in records if x["path"].startswith("supabase/migrations/")]
    need(len(migrations) == 27, "migration-count")
    actual = sorted(p.name for p in (repo / "supabase/migrations").glob("*.sql"))
    need(actual == sorted(Path(x["path"]).name for x in migrations), "migration-inventory")
    need(len({(x["scope"], x["path"]) for x in records}) == len(records), "duplicate-source")
    return records
...
def install(db, repo, password, identifier, major):
    ...
    with db.transaction():
        ...
        for path in ("tools/private-learning-owner-fence/owner-fence.sql", "tools/private-learning-operation-journal/operation-journal.sql"):
            db.execute((repo / path).read_text(encoding="utf-8"), prepare=False)
