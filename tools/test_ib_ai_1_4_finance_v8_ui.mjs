...
let equations=0;
const win={document,ECHSInstitution:{account:()=>account},FinanceLessonModels:require(fileURLToPath(new URL('data/lesson-1.4-finance-model-v8.js',base))),FinanceLessonQuestions:require(fileURLToPath(new URL('data/lesson-1.4-finance-questions-v8.js',base))),katex:{render(tex,node){math.window.katex.renderToString(tex,{throwOnError:true,strict:'ignore'});node.textContent=tex;equations++;}},addEventListener:events.addEventListener.bind(events),dispatchEvent:events.dispatchEvent.bind(events),print(){printed=true;}};
