#!/usr/bin/env python3
"""Deterministic release checks for Grade 9 lesson 2.1."""
...
LESSON_URL = "lessons/pathways/grade-9/unit-2/g9-2.1-systems-graphing-substitution.html"
LESSON = ROOT / LESSON_URL


class GradeNineSystemsGraphingSubstitutionTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.manifest = json.loads(MANIFEST.read_text(encoding="utf-8"))
        cls.html = LESSON.read_text(encoding="utf-8")
        cls.lesson = cls.manifest["paths"]["common"]["units"][2]["lessons"][0]

    def test_authoritative_manifest_row_is_truthfully_ready(self) -> None:
        self.assertEqual(self.lesson["id"], "2.1")
        self.assertEqual(self.lesson["title"], "Systems by Graphing and Substitution")
        self.assertEqual(
            self.lesson["subtopics"],
            ["Intersection meaning; graphing systems; substitution; consistency/dependence."],
        )
        self.assertIn("classify consistent/inconsistent/dependent systems", self.lesson["learningOutcomes"][0])
    ...
    def test_exact_solutions_classifications_and_context_are_consistent(self) -> None:
        for expression in (
            r"x+1=-x+5\\Rightarrow 2x=4\\Rightarrow x=2",
            r"x+(3x-1)=7\\Rightarrow 4x=8\\Rightarrow x=2",
            r"2x+2=8\\Rightarrow x=3",
            r"3x+(x-1)=11\\Rightarrow 4x=12\\Rightarrow x=3",
            r"3(2y+1)-y=13\\Rightarrow 5y=10\\Rightarrow y=2",
            r"5+2x=17+x\\Rightarrow x=12",
            r"12a+7(40-a)=360\\Rightarrow 5a=80\\Rightarrow a=16",
            r"2x+2y=12",
        ):
            self.assertIn(expression, self.html)
        for phrase in (
            "different slopes", "distinct parallel lines", "same line",
            "consistent and independent", "inconsistent", "consistent and dependent",
            "both plans cost 38 QAR at 6 hours",
        ):
            self.assertIn(phrase, self.html)
    ...
    def test_accessibility_responsive_and_visual_precision(self) -> None:
        for signal in (
            ...
            'role="img" aria-labelledby="system-title-',
            '<title id="system-title-', '<desc id="system-desc-',
            "Two lines intersect once", "Two distinct parallel lines",
            "Two equivalent equations graph as the same line",
        ):
            self.assertIn(signal, self.html)
