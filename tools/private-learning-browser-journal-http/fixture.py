"""Browser integration fixture; synthetic-only, importing never connects."""
import base64
import hashlib
import hmac
import ipaddress
...
IMAGES = {"db": None, "rest": "postgrest/postgrest@sha256:c9dc201e555f5d8e37e7f39cdd4df0229774996e213bfd7de8d10ac609030f2c"}
DB_IMAGES = {15: "postgres@sha256:9b1d34adbce1dd07ee6e94b4a2cf698884b89bd44a6c9c12f5da8f3acbfe4957", 17: "postgres@sha256:67f41722b7a8cbdb868a44a4995c846eddfdc2973bccb291ce937dce88ad5675"}
DATABASE = "echs_journal_http_fixture"


def image_tags(major):
    need(type(major) is int and major in (15, 17), "postgres-matrix")
    return {"db": DB_IMAGES[major], "rest": IMAGES["rest"]}
...
def source_receipt(repo):
    return provenance.inputs(repo)["files"]
...
def install(db, repo, password, identifier, major):
    ...
    need(db.execute("select shobj_description(oid,'pg_database') from pg_database where datname=current_database()").fetchone()[0] == project(identifier), "owned-database-before-install")
    ...
    with db.transaction():
        ...
        provenance.validate_sql(repo)
        db.execute((repo / "tools/private-learning-owner-fence-retry/owner-fence.sql").read_text(encoding="utf-8"), prepare=False)
