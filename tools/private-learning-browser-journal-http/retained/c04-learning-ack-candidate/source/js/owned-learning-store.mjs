// Isolated T3 successor: exact trusted delivery receipts and native atomic ACK; no active adoption.
...
import {acknowledgeIntent} from './journal/pending-intent.mjs';
import {SYNC_LIMITS,CONTEXT_CONTRACT,LINEAGE_CONTRACT,byteLength,retainedBytes,scopeOf,observation,checkContext,checkPreparedOrigin,checkLineage,checkBinding,makeBinding,makeWire,checkWire,acknowledgeWire} from './wire-binding-model.mjs';
export const JOURNAL_DATABASE = 'echs-owned-learning-ack-candidate-v1';
...
export async function openOwnedLearningStore({authority,indexedDB=globalThis.indexedDB,crypto=globalThis.crypto,
  ...
  practiceURL=new URL('../question-bank/practice.html',import.meta.url).href,observationPort=null,deliveryPort=null}={}){
  ...
  async function installRemoteContext(options={}){
    ...
    finally{alive=false;ticketController.abort();controller.signal.removeEventListener('abort',cancelTicket);current()}
  }
  ...
  function proofWalk(){return {cache:new Map(),visiting:new Set(),count:0,bytes:0}}
  function originCoverage(s,guard,value,done){
    const {bundle,binding:b}=value;
    readContext(s,guard,b.origin.context_id,context=>readCoverage(s,guard,context,coverage=>{
      if(!context||context.generation!==b.origin.generation||context.installed_command_revision>=bundle.command_revision||bundle.records.length&&context.installed_row_revision>=bundle.records[0].local_revision)fail('invalid_context_origin');
      bundle.records.forEach((row,i)=>{const base=b.origin.rows[i];if(base.type==='missing'&&(coverage.get(JSON.stringify([row.kind,row.record_id]))||coverage.get('*'))!==base.context_id)fail('invalid_context_origin')});
      done(coverage);
    }));
  }
  function resolveBases(s,guard,value,walk,done){
    const {bundle,binding:b}=value;
    originCoverage(s,guard,value,coverage=>{
      const ancestry=new Set(coverage.values()),resolved=[];let index=0;
      const next=()=>{if(index===bundle.records.length){done(resolved);return}const i=index++,row=bundle.records[i],base=b.origin.rows[i];
        if(base.type==='missing'){
          if((coverage.get(JSON.stringify([row.kind,row.record_id]))||coverage.get('*'))!==base.context_id)fail('invalid_context_origin');resolved.push(clone(base));next();return;
        }if(base.type==='append'){resolved.push(clone(base));next();return}
        if(base.type!=='predecessor')fail('predecessor_ack_required');
        readStoredBundle(s,guard,base.bundle_id,previous=>{
          const source=previous?.bundle.records[base.source_record_index],origin=previous?.binding?.origin;
          if(!source||!origin||previous.bundle.command_revision>=bundle.command_revision||source.kind!==row.kind||source.record_id!==row.record_id||source.local_revision!==base.local_revision||origin.generation!==b.origin.generation||!ancestry.has(origin.context_id))fail('predecessor_source');
          if(previous.binding.state!=='acknowledged')fail('predecessor_ack_required');
          readValidatedWire(s,guard,previous.binding.operation_id,walk,known=>{
            if(!known||known.bundle.id!==base.bundle_id||known.binding.state!=='acknowledged')fail('predecessor_receipt');
            const receipt=JSON.parse(known.wire.pending_intent.receipt),record=receipt.records[base.source_record_index];
            if(!record||record.kind!==row.kind||record.record_id!==row.record_id||record.local_revision!==base.local_revision||record.record_generation!==b.origin.generation||receipt.accepted_generation!==b.origin.generation||record.disposition!=='put')fail('predecessor_receipt');
            resolved.push({...base,operation_id:known.wire.id,record_revision:record.record_revision,record_generation:record.record_generation,accepted_generation:receipt.accepted_generation});next();
          },true);
        });
      };next();
    });
  }
  function readValidatedWire(s,guard,id,walk,done,terminal=false){
    if(walk.cache.has(id)){done(walk.cache.get(id));return}if(walk.visiting.has(id)||++walk.count>129)fail('wire_proof_bound');walk.visiting.add(id);
    const r=s.wire.get([...scope,id]);r.onsuccess=guard(()=>{if(r.result===undefined){walk.visiting.delete(id);done(null);return}
      const raw=clone(r.result,SYNC_LIMITS.wireBytes,2000000);walk.bytes+=retainedBytes(raw);if(walk.bytes>SYNC_LIMITS.wireBytes)fail('wire_proof_bound');
      readStoredBundle(s,guard,raw.bundle_id,value=>{if(!value?.binding)fail('wire_binding');const wire=model(()=>checkWire(raw,binding,value.bundle,value.binding));
        const finish=()=>{const result={...value,wire};walk.visiting.delete(id);walk.cache.set(id,result);done(result)};
        // An acknowledged first-apply receipt is a terminal server-CAS witness.
        // Revalidate its raw/source/receipt, not its entire historical ancestry.
        if(terminal){if(value.binding.state!=='acknowledged')fail('predecessor_ack_required');originCoverage(s,guard,value,finish);return}
        resolveBases(s,guard,value,walk,resolution=>{if(canonical(resolution)!==canonical(wire.resolution))fail('wire_resolution');finish()});
      });
    });
  }
  function readWireIntent(s,guard,id,done){readValidatedWire(s,guard,id,proofWalk(),value=>done(value?.wire??null))}
  ...
  function materializeNext(){
    current();const operationId=crypto.randomUUID();return transact('readwrite',(s,guard,set)=>{const owner=s.owners.get(scope);owner.onsuccess=guard(()=>{
      const m=meta(owner.result);if(m.in_flight_operation_id!==null){readValidatedWire(s,guard,m.in_flight_operation_id,proofWalk(),value=>{if(!value)fail('missing_wire_intent');if(value.binding.state!=='bound'||value.bundle.command_revision!==m.queue_cursor+1)fail('wire_pointer');set(freeze({status:'bound',duplicate:true,transport:'held',intent:value.wire}))});return}
      ...
      cursor.onsuccess=guard(()=>{const entry=cursor.result;if(!entry){if(m.queue_cursor<m.bundle_count)fail('invalid_store');finish({status:'empty'});return}if(++scanned>100){finish({status:'held',reason:'page_limit'});return}
        const wrapper=checkedBundle(entry.value);if(wrapper.revision!==m.queue_cursor+1)fail('invalid_store');readStoredBundle(s,guard,wrapper.id,value=>{
          ...
          if(b.state==='bound')fail('wire_pointer');if(b.state==='acknowledged')fail('ack_queue');if(b.state==='held'){finish({status:'held',reason:b.hold_reason,bundle_id:wrapper.id});return}
          resolveBases(s,guard,value,proofWalk(),resolution=>{
            let intent;try{intent=model(()=>makeWire(value.bundle,b,binding,operationId,resolution))}catch(error){if(!(error instanceof SyncJournalError))throw error;const reason=['predecessor_ack_required','wire_record_limit','wire_row_limit','wire_byte_limit'].includes(error.code)?error.code:'wire_contract';const held={...b,state:'held',hold_reason:reason};charge(m,'binding',held,b);s.bindings.put(held);m.blocked_at_bundle_id??=b.id;advanceSync(m);finish({status:'held',reason,bundle_id:b.id});return}
  ...
  function validateLatestSource(s,guard,latest,wireValue,index,done){
    const source=wireValue.bundle.records[index],r=s.lineage.get([...scope,source.kind,source.record_id]);
    r.onsuccess=guard(()=>{
      const link=model(()=>checkLineage(r.result===undefined?undefined:clone(r.result,4096),binding,source.kind,source.record_id));
      if(!link||link.local_revision!==latest.revision)fail('ack_latest_source');
      readStoredBundle(s,guard,link.bundle_id,known=>{
        const row=known?.bundle.records[link.source_record_index],origin=known?.binding?.origin;
        if(!row||!origin||row.kind!==source.kind||row.record_id!==source.record_id||row.local_revision!==latest.revision||row.value_json!==JSON.stringify(latest.row)||origin.context_id!==link.context_id||origin.generation!==link.generation)fail('ack_latest_source');
        if(latest.revision===source.local_revision){
          if(link.bundle_id!==wireValue.bundle.id||link.source_record_index!==index||row.value_json!==source.value_json)fail('ack_source_changed');
        }else if(latest.revision<source.local_revision||known.bundle.command_revision<=wireValue.bundle.command_revision)fail('ack_latest_source');
        done();
      });
    });
  }
  function persistDelivery(id,exactRaw,evidence){
    return transact('readwrite',(s,guard,set)=>{const owner=s.owners.get(scope);owner.onsuccess=guard(()=>{
      const m=meta(owner.result);readValidatedWire(s,guard,id,proofWalk(),value=>{
        if(!value||value.wire.exact_raw!==exactRaw)fail('wire_changed');
        const accepted=model(()=>acknowledgeWire(value.wire,binding,value.bundle,value.binding,evidence));
        if(!accepted){set(freeze({status:'pending',acknowledged:false,operation_id:id,reason:'receipt_rejected'}));return}
        const generationBlocked=m.generation_blocked||accepted.current.reset_generation>value.binding.origin.generation;
        if(value.binding.state==='acknowledged'){
          if(m.queue_cursor<value.bundle.command_revision||m.in_flight_operation_id===id)fail('ack_queue');
          if(generationBlocked!==m.generation_blocked){m.generation_blocked=generationBlocked;advanceSync(m);s.owners.put(m)}
          set(freeze({status:'acknowledged',acknowledged:true,duplicate:true,operation_id:id,cleared:0,preserved:0,generation_blocked:generationBlocked,receipt:value.wire.pending_intent.receipt}));return;
        }
        if(m.in_flight_operation_id!==id||value.bundle.command_revision!==m.queue_cursor+1)fail('ack_queue');
        const clear=[];let index=0,preserved=0;
        const next=()=>{
          if(index===value.bundle.records.length){
            const nextBinding={...value.binding,state:'acknowledged'};
            // Charge all retained first evidence before issuing a pending-row delete.
            charge(m,'wire',accepted.wire,value.wire);charge(m,'binding',nextBinding,value.binding);
            const bytes=clear.reduce((n,row)=>n+row.bytes,0);if(m.count<clear.length||m.bytes<bytes)fail('invalid_store');
            for(const row of clear)s.records.delete([...scope,row.kind,row.id]);
            m.count-=clear.length;m.bytes-=bytes;m.queue_cursor=value.bundle.command_revision;m.in_flight_operation_id=null;m.generation_blocked=generationBlocked;advanceSync(m);
            s.wire.put(accepted.wire);s.bindings.put(nextBinding);s.owners.put(m);
            set(freeze({status:'acknowledged',acknowledged:true,duplicate:false,operation_id:id,cleared:clear.length,preserved,generation_blocked:generationBlocked,receipt:accepted.wire.pending_intent.receipt}));return;
          }
          const i=index++,source=value.bundle.records[i],r=s.records.get([...scope,source.kind,source.record_id]);r.onsuccess=guard(()=>{
            const latest=record(r.result,source.kind,source.record_id);
            // Missing rows may need a future reset/deletion policy; retain this ACK.
            if(!latest||latest.revision<source.local_revision)fail('ack_pending_missing');
            if(latest.revision>m.revision||m.count<1||m.bytes<latest.bytes)fail('invalid_store');
            if(latest.revision===source.local_revision&&JSON.stringify(latest.row)!==source.value_json)fail('ack_source_changed');
            validateLatestSource(s,guard,latest,value,i,()=>{if(latest.revision===source.local_revision)clear.push(latest);else preserved++;next()});
          });
        };next();
      });
    })});
  }
  async function deliverNext(options={}){
    current();if(typeof deliveryPort?.deliver!=='function')fail('delivery_port_unavailable');const o=clone(options,4096);namedObject(o,['operation_id']);
    if(o.operation_id!==undefined&&(typeof o.operation_id!=='string'||!UUID.test(o.operation_id)))fail('invalid_command');
    let intent;if(o.operation_id!==undefined){intent=await wireIntent(o.operation_id);if(!intent)fail('missing_wire_intent')}
    else{const selected=await materializeNext();if(selected.status!=='bound')return selected;intent=selected.intent}
    const captured=clone(current()),id=intent.id,exactRaw=intent.exact_raw,ticket=new AbortController(),deadline=performance.now()+10000;
    const cancel=()=>ticket.abort(controller.signal.reason);controller.signal.addEventListener('abort',cancel,{once:true});if(controller.signal.aborted)cancel();
    let alive=true,timer,abort,response;
    const remaining=()=>{const left=deadline-performance.now();if(!alive||ticket.signal.aborted||left<=0)fail('delivery_deadline');return left};
    const cancelBody=r=>{try{void r?.body?.cancel().catch(()=>{})}catch{}};
    try{
      const pending=Promise.resolve().then(()=>{current();remaining();return deliveryPort.deliver({route:'apply',requestRaw:exactRaw,signal:ticket.signal})}).then(r=>{if(!alive||ticket.signal.aborted){cancelBody(r);fail('delivery_deadline')}return r});pending.catch(()=>{});
      response=await Promise.race([pending,new Promise((_,reject)=>{abort=()=>reject(new SyncJournalError(reason||'delivery_deadline'));ticket.signal.addEventListener('abort',abort,{once:true});if(ticket.signal.aborted)abort();timer=setTimeout(()=>reject(new SyncJournalError('delivery_deadline')),remaining())})]);
      clearTimeout(timer);ticket.signal.removeEventListener('abort',abort);current();remaining();if(!equal(current(),captured))fail('owner_changed');
      if(!(response instanceof Response)||response.redirected||response.status!==200||!/^application\/json(?:\s*;\s*charset=utf-8)?$/i.test(response.headers.get('content-type')||'')||response.headers.has('content-encoding')&&response.headers.get('content-encoding')!=='identity')fail('delivery_response');
      const length=response.headers.get('content-length');if(length!==null&&(!/^(0|[1-9][0-9]*)$/.test(length)||Number(length)>1048576))fail('delivery_response');
      const replyRaw=await readWire(response.body,{signal:ticket.signal,timeoutMs:remaining()});current();remaining();if(!equal(current(),captured))fail('owner_changed');
      const validated=acknowledgeIntent(intent.pending_intent,{route:'apply',requestRaw:exactRaw,status:200,replyRaw,activeOwner:binding});
      if(!validated.acknowledged)return freeze({status:'pending',acknowledged:false,operation_id:id,reason:'receipt_rejected'});
      const result=await persistDelivery(id,exactRaw,{route:'apply',request_raw:exactRaw,status:200,reply_raw:replyRaw});
      current();if(result.generation_blocked)viewOrigin={context_id:null,generation:null,reset_barrier_id:viewOrigin.reset_barrier_id};return result;
    }catch(error){
      current();if(error instanceof SyncJournalError&&['timeout','storage_error','invalid_store','wire_changed','wire_resolution','wire_binding','wire_receipt','ack_queue','ack_pending_missing','ack_source_changed','ack_latest_source','wire_full','binding_full','wire_proof_bound'].includes(error.code))throw error;
      return freeze({status:'pending',acknowledged:false,operation_id:id,reason:error instanceof SyncJournalError||error instanceof JournalError?error.code:'delivery_failed'});
    }finally{alive=false;clearTimeout(timer);ticket.signal.removeEventListener('abort',abort);ticket.abort();controller.signal.removeEventListener('abort',cancel);cancelBody(response);current()}
  }
  ...
  return Object.freeze({prepare,commit,lookup,refresh,projection,query,snapshot,sourceBundle,sourceBundles,installRemoteContext,materializeNext,wireIntent,bundleOrigin,deliverNext,binding:()=>freeze(clone(current())),
