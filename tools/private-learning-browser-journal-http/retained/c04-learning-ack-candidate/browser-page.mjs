...
import {CONTRACT as SERVER_CONTRACT,LIMITS as SERVER_LIMITS,canonical} from './source/js/journal/contract.mjs';
import {makeWire,retainedBytes} from './source/js/wire-binding-model.mjs';
...
const names=['owners','records','attempts','states','commands','history','bundles','contexts','bindings','lineage','wire'];
let remote={generation:0,revision:0,heads:{}},observationHook=null;const observed=[];
let deliveryHook=null;const delivered=[],syntheticOperations=new Map();
function syntheticReply(raw){
 const request=JSON.parse(raw),known=syntheticOperations.get(request.operation_id);
 if(known&&known.raw!==raw)return new Response('{}',{status:409,headers:{'content-type':'application/json'}});
 let receipt=known?.receipt;
 if(!receipt){
  if(request.reset_generation!==remote.generation)return new Response('{}',{status:409,headers:{'content-type':'application/json'}});
  for(const row of request.records)if(row.kind!=='attempts'&&(remote.heads[JSON.stringify([row.kind,row.record_id])]?.revision??0)!==row.expected_revision)return new Response('{}',{status:409,headers:{'content-type':'application/json'}});
  receipt={contract:'echs.learning.server-receipt.v1',operation_id:request.operation_id,incarnation_id:request.incarnation_id,adoption_epoch:1,accepted_generation:request.reset_generation,owner_revision:++remote.revision,request_sha256:'a'.repeat(64),accepted_at:'2026-09-12T12:00:00.000000Z',durability:'committed',grading_authoritative:false,action:'commit',records:request.records.map((row,i)=>({request_index:i,kind:row.kind,record_id:row.record_id,local_revision:row.local_revision,record_revision:row.kind==='attempts'?1:row.expected_revision+1,record_generation:request.reset_generation,value_sha256:'b'.repeat(64),disposition:row.kind==='attempts'?'appended':'put'})),reset_to:null};
  request.records.forEach((row,i)=>{remote.heads[JSON.stringify([row.kind,row.record_id])]={revision:receipt.records[i].record_revision,value:row.value,value_sha256:'b'.repeat(64),deleted:false}});syntheticOperations.set(request.operation_id,{raw,receipt});
 }
 return new Response(JSON.stringify({ok:true,contract:SERVER_CONTRACT,replayed:!!known,receipt,current:{reset_generation:remote.generation,owner_revision:remote.revision}}),{headers:{'content-type':'application/json'}});
}
async function deliverSynthetic(request){delivered.push({route:request.route,requestRaw:request.requestRaw});if(deliveryHook){const result=await deliveryHook(request);if(result!==undefined)return result}if(request.route!=='apply')throw Error('Synthetic port supports apply only');return syntheticReply(request.requestRaw)}
async function readObservation(request){observed.push({route:request.route,requestRaw:request.requestRaw});if(observationHook){const response=await observationHook(request);if(response!==undefined)return response}
 const req=JSON.parse(request.requestRaw);let data;
 if(request.route==='state')data={ok:true,contract:SERVER_CONTRACT,organization_id:identity.organization_id,account_id:identity.account_id,incarnation_id:identity.incarnation_id,adoption_epoch:1,reset_generation:remote.generation,owner_revision:remote.revision,limits:SERVER_LIMITS,grading_authoritative:false};
 else if(request.route==='heads')data={ok:true,contract:SERVER_CONTRACT,current:{reset_generation:remote.generation,owner_revision:remote.revision},records:req.keys.map(key=>({...key,revision:0,deleted:false,value:null,value_sha256:null,...remote.heads[JSON.stringify([key.kind,key.record_id])]}))};
 else throw Error('Synthetic observation port is read-only');return new Response(JSON.stringify(data),{status:200,headers:{'content-type':'application/json'}});
}
...
const f=window.fixture={DATABASE:JOURNAL_DATABASE,LIMITS:BUNDLE_LIMITS,SERVER_LIMITS,canonical,makeWire,retainedBytes,stores,names,q,authority,now:()=>now,observed,delivered,syntheticReply,
 deliveryHook(fn){deliveryHook=fn},
 remote(value){Object.assign(remote,value)},observationHook(fn){observationHook=fn},
 async open({prior=false,initialize=true,...extra}={}){const s=await (prior?openOriginal:openOwnedLearningStore)({authority:authority(prior),observationPort:{read:readObservation},deliveryPort:{deliver:deliverSynthetic},clock:()=>time,random:()=>seed,now:()=>now,practiceURL:location.origin+'/question-bank/practice.html',...extra});stores.push(s);if(initialize)await s.commit(s.prepare('initialize'));return stores.length-1},
 ...
 async compare(method,args){const command=stores[0].prepare(method,args),{origin,...originalCommand}=command;const a=await stores[0].commit(command),b=await stores[1].commit({...originalCommand,owner:stores[1].binding()});
 ...
 injectDelete(fn){const original=IDBObjectStore.prototype.delete;let done=false;IDBObjectStore.prototype.delete=function(...args){const r=original.apply(this,args);if(!done&&this.name==='records'){done=true;IDBObjectStore.prototype.delete=original;r.addEventListener('success',()=>fn(this.transaction),{once:true})}return r}},
