...
import {createPendingIntent} from './journal/pending-intent.mjs';
...
export const BINDING_CONTRACT='echs.learning.bundle-binding.v1';
export const LINEAGE_CONTRACT='echs.learning.source-lineage.v1';
export const WIRE_CONTRACT='echs.learning.bound-wire.v1';
...
export function checkBinding(v,scope,bundle){
 ...
 need(v.contract===BINDING_CONTRACT&&v.id===bundle.id&&v.command_revision===bundle.command_revision&&['local-only','unbound','held','bound'].includes(v.state)&&nullableId(v.operation_id),'binding_shape');
 ...
 need(v.state==='bound'?uuid(v.operation_id)&&v.hold_reason===null:v.operation_id===null,'binding_operation');
...
export function makeWire(bundle,binding,scope,operationId){
 ...
 need(binding.origin.rows.every(r=>r.type==='append'||r.type==='missing'),'predecessor_ack_required');
 need(bundle.records.length>=1&&bundle.records.length<=128,'wire_record_limit');
 const records=bundle.records.map((row,i)=>{need(byteLength(row.value_json)<=65536,'wire_row_limit');const base=binding.origin.rows[i];const metadata={kind:row.kind,record_id:row.record_id,local_revision:row.local_revision,action:row.kind==='attempts'?'append':'put'};
  if(row.kind!=='attempts')metadata.expected_revision=base.expected_revision;
 ...
 const pending=createPendingIntent(raw,scope);return {contract:WIRE_CONTRACT,...scope,id:operationId,bundle_id:bundle.id,exact_raw:raw,raw_bytes:byteLength(raw),pending_intent:pending};
}
export function checkWire(v,scope,bundle,binding){
 closed(v,['contract',...scopeKeys,'id','bundle_id','exact_raw','raw_bytes','pending_intent']);scoped(v,scope);
 need(v.contract===WIRE_CONTRACT&&v.id===binding.operation_id&&v.bundle_id===bundle.id&&binding.state==='bound','wire_binding');
 const rebuilt=makeWire(bundle,{...binding,state:'unbound',operation_id:null},scope,v.id);
 need(v.exact_raw===rebuilt.exact_raw&&v.raw_bytes===rebuilt.raw_bytes&&canonical(v.pending_intent)===canonical(rebuilt.pending_intent),'wire_changed');return v;
}
