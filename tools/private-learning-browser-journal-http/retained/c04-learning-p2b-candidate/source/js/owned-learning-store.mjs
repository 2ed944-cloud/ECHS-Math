// Isolated P2a copy of frozen P1: original transitions and fixed practice flows.
...
export const JOURNAL_DATABASE = 'echs-owned-learning-p2b-practice-candidate-v1';
...
const STORES = ['owners','records','attempts','states','commands','history'];
...
function identity(value,now){
  const out=clone(value,4096);closed(out,['kind','organization_id','account_id','role','status','expires_at','session_id','epoch']);
  if(out.kind!=='account'||!UUID.test(out.organization_id)||!UUID.test(out.account_id)||out.role!=='student'||out.status!=='active'||
...
export async function openOwnedLearningStore({authority,indexedDB=globalThis.indexedDB,crypto=globalThis.crypto,
  ...
  if(typeof authority?.capture!=='function'||typeof authority?.verify!=='function'||typeof authority?.subscribe!=='function'||
    typeof indexedDB?.open!=='function'||typeof crypto?.subtle?.digest!=='function'||typeof IDBKeyRange?.only!=='function'||
  ...
  const scope=[owner.organization_id,owner.account_id];
  function dispose(code='disposed'){
    if(reason)return;reason=code;owner=null;view=null;receipts.clear();
  ...
  function meta(value){
    if(value===undefined)return{contract:JOURNAL_CONTRACT,organization_id:scope[0],account_id:scope[1],revision:0,count:0,bytes:0,attempt_ids:0};
    const v=clone(value,4096);closed(v,['contract','organization_id','account_id','revision','count','bytes','attempt_ids']);
    if(v.contract!==JOURNAL_CONTRACT||v.organization_id!==scope[0]||v.account_id!==scope[1]||
      ...
      !Number.isSafeInteger(v.attempt_ids)||v.attempt_ids<0||v.attempt_ids>JOURNAL_LIMITS.attemptIds)fail('invalid_store');return v;
  }
  function record(value,kind,id){
    if(value===undefined)return null;const v=clone(value,JOURNAL_LIMITS.rowBytes+2048);
    closed(v,['contract','source_contract','organization_id','account_id','kind','id','revision','bytes','row']);
    if(v.contract!==JOURNAL_CONTRACT||v.source_contract!==SOURCE_CONTRACT||v.organization_id!==scope[0]||v.account_id!==scope[1]||
  ...
  function stamp(value,id){
    if(value===undefined)return null;const v=clone(value,4096);closed(v,['contract','organization_id','account_id','id','sha256']);
    if(v.contract!==JOURNAL_CONTRACT||v.organization_id!==scope[0]||v.account_id!==scope[1]||v.id!==id||!HASH.test(v.sha256))fail('invalid_store');return v;
  }
  function transact(mode,work){
    current();return new Promise((resolve,reject)=>{
      ...
      try{
        tx=db.transaction(STORES,mode);transactions.add(tx);
  ...
  try{
    ...
    if(verified.contract!=='echs.owner-authority.v1'||verified.verified!==true||!equal(identity(verified.identity,now),owner))fail('verification_failed');
    await bounded(new Promise((resolve,reject)=>{
      ...
      request.onupgradeneeded=event=>{
        try{current();if(event.oldVersion!==0)fail('invalid_store');const d=request.result;
          d.createObjectStore('owners',{keyPath:['organization_id','account_id']});
          const records=d.createObjectStore('records',{keyPath:['organization_id','account_id','kind','id']});records.createIndex('owner',['organization_id','account_id']);
          d.createObjectStore('attempts',{keyPath:['organization_id','account_id','id']});
          d.createObjectStore('states',{keyPath:['organization_id','account_id']});
          d.createObjectStore('commands',{keyPath:['organization_id','account_id','id']});
          d.createObjectStore('history',{keyPath:['organization_id','account_id','id']});
      ...
      request.onsuccess=()=>{
        ...
        try{current();if(value.version!==1||!equal([...value.objectStoreNames].sort(),[...STORES].sort()))fail('invalid_store');
          ...
          for(const [name,path]of [['owners',['organization_id','account_id']],['records',['organization_id','account_id','kind','id']],['attempts',['organization_id','account_id','id']],['states',['organization_id','account_id']],['commands',['organization_id','account_id','id']],['history',['organization_id','account_id','id']]]){
            const s=tx.objectStore(name);if(!equal(s.keyPath,path)||s.autoIncrement)fail('invalid_store');
          }
          const idx=tx.objectStore('records').index('owner');if(!equal(idx.keyPath,['organization_id','account_id'])||idx.unique||idx.multiEntry)fail('invalid_store');
          db=value;db.onversionchange=()=>dispose('version_changed');db.onclose=()=>dispose('storage_closed');resolve();
  ...
  function writeRows(s,guard,rows,set){
        const get=s.owners.get(scope);get.onsuccess=guard(()=>{
          const m=meta(get.result);let index=0,changed=0,unchanged=0;
          const next=()=>{
            if(index===rows.length){s.owners.put(m);set({changed,unchanged,pending:m.count});return}
            ...
            read.onsuccess=guard(()=>{
              ...
              const put=()=>{
                ...
                s.records.put({contract:JOURNAL_CONTRACT,source_contract:SOURCE_CONTRACT,organization_id:scope[0],account_id:scope[1],kind:item.kind,id:item.id,revision:m.revision,bytes:item.bytes,row:item.row});changed++;next();
              };
              ...
              const remembered=s.attempts.get([...scope,item.id]);remembered.onsuccess=guard(()=>{
                ...
                if(!prior){if(old)fail('invalid_store');if(++m.attempt_ids>JOURNAL_LIMITS.attemptIds)fail('journal_full');
                  s.attempts.add({contract:JOURNAL_CONTRACT,organization_id:scope[0],account_id:scope[1],id:item.id,sha256:item.sha256});}
                put();
              });
            });
          };next();
  ...
  function engine(value){
    if(value===undefined)return{contract:ENGINE_CONTRACT,organization_id:scope[0],account_id:scope[1],revision:0,data:{},command_count:0,command_bytes:0,history_count:0,history_bytes:0};
    const v=clone(value,STATE_LIMIT,2000000);
    closed(v,['contract','organization_id','account_id','revision','data','command_count','command_bytes','history_count','history_bytes']);
    if(v.contract!==ENGINE_CONTRACT||v.organization_id!==scope[0]||v.account_id!==scope[1]||!plain(v.data)||Object.keys(v.data).some(k=>!DOMAIN_KEYS.includes(k))||
      !Number.isSafeInteger(v.revision)||v.revision<0||v.revision>=Number.MAX_SAFE_INTEGER||
  ...
  function commandRecord(value){
    ...
    closed(v,['contract','organization_id','account_id','id','sha256','receipt']);
    if(v.contract!==ENGINE_CONTRACT||v.organization_id!==scope[0]||v.account_id!==scope[1]||!UUID.test(v.id)||!HASH.test(v.sha256))fail('invalid_store');
  ...
  async function lookup(operationId){
    ...
    const found=await transact('readonly',(s,guard,set)=>{const r=s.commands.get([...scope,operationId]);r.onsuccess=guard(()=>set(commandRecord(r.result)))});
      ...
      const result=await transact('readwrite',(s,guard,set)=>{
        const prior=s.commands.get([...scope,cmd.operation_id]);prior.onsuccess=guard(()=>{
          ...
          read.onsuccess=guard(()=>{
            ...
            if(remembered){if(remembered.receipt.revision>existing.revision)fail('invalid_store');set({engine:existing,receipt:{...remembered.receipt,duplicate:true,effects:[]}});return}
            ...
            const apply=()=>{
              ...
              const saved={contract:ENGINE_CONTRACT,organization_id:scope[0],account_id:scope[1],id:cmd.operation_id,sha256:fingerprint,receipt};
              ...
              if(candidateAttempt){
                const archived={contract:ENGINE_CONTRACT,organization_id:scope[0],account_id:scope[1],id:candidateAttempt.id,sha256:candidateAttempt.sha256,row:candidateAttempt.row};
              ...
              writeRows(s,guard,rows,()=>{s.states.put(next);s.commands.add(saved);set({engine:next,receipt})});
            };
  ...
  return Object.freeze({prepare,commit,lookup,refresh,projection,query,snapshot,binding:()=>freeze(clone(current())),
