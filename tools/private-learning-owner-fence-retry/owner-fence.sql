...
begin
  ...
  if not found then raise exception 'Owner barrier unavailable' using errcode='55000';end if;
  ...
  if not found then raise exception 'Fresh owner fence required' using errcode='55000';end if;
...
begin
  ...
  for owner_id in select distinct value from unnest(array[old_owner,new_owner]) value where value is not null order by value loop
    ...
    if not seen then
      ...
      if not seen then raise exception 'Fresh owner fence required' using errcode='55000';end if;
    end if;
  end loop;
...
begin
  ...
  if not found then raise exception 'Fresh owner barrier required' using errcode='55000';end if;
