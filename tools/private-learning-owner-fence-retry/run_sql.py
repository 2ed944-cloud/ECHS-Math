...
from contract import HERE,REPO,FENCE,sources,snapshot,unchanged,digest,connection_guard,connected,load_fence,planned_labels,sql_buffers,checkout_sources,journal_fixture,retained_pin
...
def main():
    ...
    if not args.execute:
        print(json.dumps({'status':'SOURCE PREFLIGHT ONLY; POSTGRESQL NOT EXECUTED','prerequisites':72,'original_migrations':27,'reviewed_owner_fence_sources':12,'postgres_required':args.postgres_major,
    ...
    report={'contract':'echs.c04.owner-fence-retry-sql.v1','status':'RUNNING; NOT ACCEPTED','source_files':expected,'migrations':migration_rows,
    ...
    try:
        ...
        class CandidateConnection(psycopg.Connection):
            def execute(self,query,*params,**kwargs):
                ...
                if type(query) is str and query==trigger:
                    ...
                    injections.append({'after_migration':migration_rows[-1],'successor_fence_sha256':digest(fence_sql.encode()),'journal_sha256':retained_pin('tools/private-learning-operation-journal/operation-journal.sql')['sha256'],
        ...
        verified()
        exercise=journal_fixture().exercise
        with connect() as db:
        ...
        report.update(status='ACTUAL SUCCESSOR FENCE SQL448 PASS; NO ACTIVE API OR ADOPTION',database_executed=True,
