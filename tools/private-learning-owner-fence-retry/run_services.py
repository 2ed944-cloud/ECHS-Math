...
def verify(repo, published=False):
    ...
    need(manifest["contract"] == "echs.c04.owner-fence-retry-sources.v1", "manifest")
    paths = set()
    ...
    need(WORKFLOW in paths and PREFIX + "run_services.py" in paths and PREFIX + "input-pins.json" in paths, "source-closure")
...
def main():
    ...
    report = {"contract": "echs.c04.owner-fence-retry-service-run.v1", "status": "RUNNING; NOT ACCEPTED", "run_id": identifier, "postgres_major": args.postgres_major, "production_calls": 0, "hosted_edge_executed": False, "tls_executed": False, "browser_persistence_executed": False, "service_start_attempted": False, "cleanup_complete": False}
    def save(path, value):
    ...
    try:
        ...
        report["groups"] = 20
        stage = "focused-sql-r01-r09"
        command([sys.executable, str(HERE / "run_retry_sql.py"), str(private_config),
                 str(run_dir / "retry-sql-results.json"), str(args.postgres_major)], timeout=120)
        focused = json.loads((run_dir / "retry-sql-results.json").read_text())
        need(focused["status"] == "ACTUAL FOCUSED SQL R01-R09 PASS"
             and len(focused["checks"]) == 9 and focused["source_unchanged"] is True, "focused-sql-acceptance")
        report["focused_sql_groups"] = 9
        stage = "retry-http-r10-r11"
        from test_retry_http import exercise as retry_http
        def stop_rest():
            value = inspect_container("rest")
            cleanup_identity(value, identifier, "rest")
            docker("stop", "--time", "2", value["Id"])
            stopped = inspect_container("rest")
            cleanup_identity(stopped, identifier, "rest")
            need(stopped["State"]["Running"] is False, "owned-rest-stop")
            return True
        try:
            retry_result = retry_http(config, args.postgres_major, stop_rest)
        except Exception as error:
            save(run_dir / "retry-http-results.json", {"status": "FAIL; NO ACCEPTANCE", "failure_type": type(error).__name__, "production_calls": 0})
            raise
        save(run_dir / "retry-http-results.json", retry_result)
        need(retry_result["status"] == "ACTUAL RETRY HTTP PASS; OWNED POSTGREST STOPPED"
             and len(retry_result["checks"]) == 2 and retry_result["owned_backend_count_after_stop"] == 0
             and retry_result["http_worker_reaped"] is True, "retry-http-acceptance")
        need(verify(repo, True) == receipt, "final-successor-source-drift")
        report["retry_http_groups"] = 2
        report["r12_reference"] = "The unchanged twenty J049 HTTP cases above are counted once."
        report["status"] = "ACTUAL OWNER FENCE RETRY SERVICES PASS"
    except Exception as error:
        ...
        artifact = Path(os.environ["RUNNER_TEMP"]) / ("private-learning-owner-fence-retry-" + str(args.postgres_major))
        need(not artifact.exists(), "fresh-artifact-directory"); artifact.mkdir(mode=0o700)
        rows = []
        for filename in ("source-receipt.json", "image-receipt.json", "http-results.json", "retry-sql-results.json", "retry-http-results.json", "run-report.json"):
    ...
    return 0 if report["status"] == "ACTUAL OWNER FENCE RETRY SERVICES PASS" and cleanup_ok else 1
