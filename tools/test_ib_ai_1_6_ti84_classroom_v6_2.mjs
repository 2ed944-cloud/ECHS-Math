...
const file=`${root}/lessons/ib-math-ai/unit-1/data/lesson-1.6-ti84-classroom-coach-v6-2.js`;
...
const sandbox={
  ...
  document:{readyState:'loading',addEventListener(){},querySelector(){return null;},querySelectorAll(){return[];},body:{}},
...
try{vm.runInContext(source,sandbox,{filename:file});}catch(error){errors.push(`Coach assembly failed: ${error.stack}`);}
...
if(metadata.release!=='6.2.0')errors.push(`Release mismatch: ${metadata.release}`);
...
for(const marker of ['lesson-1.6-ti84-classroom-coach-v6-2.css?v=6.2.0','lesson-1.6-ti84-classroom-coach-v6-2.js?v=6.2.0'])if(!html.includes(marker))errors.push(`Wrapper missing ${marker}`);
if(html.indexOf('lesson-1.6-technology-v6-gdc-external-tools.js')>html.indexOf('lesson-1.6-ti84-classroom-coach-v6-2.js'))errors.push('TI-84 coach must load after connected resource layer');
...
for(const marker of ['Teacher demo','Students follow','Exam drill','Load simulator','Manual mathematics','TI‑84 key sequence'])if(!source.includes(marker))errors.push(`Coach UI missing ${marker}`);
for(const marker of ['Third-party tool hosted by ti84calc.com','physical calculator','PlySmlt2','rref([A])','5:intersect','2:zero'])if(!source.includes(marker))errors.push(`Instructional contract missing ${marker}`);

console.log('IB AI SL Lesson 1.6 TI-84 Classroom Practice v6.2');
