    ...
    def request(
        ...
        *,
        ...
        merged = {
            ...
            "user-agent": "ECHS-Private-Bank-Importer/2.1-resumable",
        }
    ...
    with zipfile.ZipFile(args.package) as archive:
        ...
        if not args.skip_media:
            for archive_index, media_name in enumerate(media_archives, 1):
                ...
                with zipfile.ZipFile(io.BytesIO(archive.read(media_name))) as media_archive:
                    for source_path in sorted(name for name in media_archive.namelist() if not name.endswith("/")):
                        ...
                        media_rows.append({
                            ...
                            "sha256": data_hash, "mime_type": content_type, "uploaded_at": now,
                        })
                        uploaded += 1
                ...
                progress(client, args.request_id, percent, f"Media archives: {archive_index}/{len(media_archives)} · uploaded {uploaded} · reused {skipped}")
        ...
        client.patch("private_bank_packages", {"organization_id": args.organization_id, "bank_code": bank_code}, {
            "deployment_state": state, "media_count": total_media, "updated_at": datetime.now(timezone.utc).isoformat(),
        })
        result = {"bank_code": bank_code, "questions": len(rows), "media_uploaded": uploaded,
                  "media_reused": skipped, "media_total": total_media, "package_sha256": package_hash,
                  "deployment_state": state, "trust_tier": "publisher_key_direct"}
