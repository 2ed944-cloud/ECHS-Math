...
const livePackages=['ECHS-BB-AT9','ECHS-BB-CA9','ECHS-BB-CA9B','ECHS-BB-ACS10'].map((bank_code,index)=>({bank_code,deployment_state:'complete-direct-upload',question_count:[4945,3604,2837,4285][index],pool_count:[355,253,282,594][index],media_count:[13439,8767,7415,8972][index],trust_default:'publisher_key_direct'}));
...
for(const device of devices){
  ...
  try{
    ...
    if(state.complete!==4||!state.text.includes('complete direct upload'))entry.errors.push('Direct live package states did not render');
    ...
    if(state.alignmentCards!==2||state.apLessons!=='50'||state.apReadiness!=='6,695'||state.apVerified!=='15,671')entry.errors.push('AP Precalculus direct mapping metrics did not render');
