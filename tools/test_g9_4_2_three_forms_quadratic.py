#!/usr/bin/env python3
"""Deterministic release checks for Grade 9 lesson 4.2."""
...
LESSON_URL = "lessons/pathways/grade-9/unit-4/g9-4.2-three-forms-quadratic.html"
LESSON = ROOT / LESSON_URL


class GradeNineThreeFormsQuadraticTests(unittest.TestCase):
    @classmethod
    ...
    def test_authoritative_manifest_row_is_truthfully_ready(self) -> None:
        self.assertEqual(self.lesson["title"], "Three Forms of a Quadratic")
        self.assertEqual(
            self.lesson["subtopics"],
            ["Standard, factored, vertex forms; information revealed; strategic conversion."],
        )
        self.assertEqual(
            self.lesson["learningOutcomes"],
            ["Choose and use the most informative quadratic form for a task."],
        )
        self.assertEqual(self.lesson["depth"], "M Prerequisite Mastery")
        self.assertEqual(
            self.lesson["requiredGrade9Coverage"],
            "Interpret each form and convert among forms efficiently when useful.",
        )
        self.assertEqual(
            self.lesson["coverageBoundary"],
            "Limited emphasis: tedious expansion beyond what supports interpretation and solving.",
        )
    ...
    def test_depth_pacing_coverage_and_boundary_are_explicit(self) -> None:
        for phrase in (
            ...
            "4 target periods",
            "4 maximum",
            "Interpret each quadratic form",
            "convert among forms efficiently",
            "Tedious expansion",
    ...
    def test_representative_conversions_are_exact(self) -> None:
        for x in (-4, 0, 1, 3, 6):
            self.assertEqual(x * x - 2 * x - 3, (x - 3) * (x + 1))
            self.assertEqual(x * x - 2 * x - 3, (x - 1) ** 2 - 4)
            self.assertEqual(-2 * (x - 1) * (x - 5), -2 * x * x + 12 * x - 10)
            self.assertEqual(2 * (x + 3) ** 2 - 8, 2 * (x + 1) * (x + 5))
            self.assertEqual(2 * x * x - 8 * x + 3, 2 * (x - 2) ** 2 - 5)
            self.assertEqual(-(x - 1) * (x - 5), -(x - 3) ** 2 + 4)
            self.assertEqual(-2 * (x - 3) ** 2 + 18, -2 * x * (x - 6))

        self.assertEqual(2 * (0 + 2) * (0 - 6), -24)
        self.assertEqual(-3 * (0 - 1) * (0 - 5), -15)
        self.assertEqual(-2 * (4 - 1) * (4 - 7), 18)
        self.assertEqual(-2 * (0 + 2) ** 2 + 5, -3)
    ...
    def test_screen_plan_is_64_meaningful_screens(self) -> None:
        ...
        for signal in (
            ...
            "Student Turn", "Standard form", "Factored form", "Vertex form",
            "Choose the form", "Differentiated practice", "AP-readiness transfer",
    ...
    def test_accessibility_responsive_and_visual_precision(self) -> None:
        ...
        for prohibited in (
            ...
            "quadratic patterns & graph features", "difference quotients",
        ):
