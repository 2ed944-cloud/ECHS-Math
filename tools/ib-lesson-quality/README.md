These checks cover the IB1.5 scope/quiz changes, IB1.6 line clipping and the scoped GDC math bridge. They do not certify production authentication, synchronization or mastery authority.

The component test reads current sources from `--repo` and exact original files from `--baseline-root`. `baseline-pins.json` lists five original paths and byte/SHA-256 pins at commit `dcc65f84dafd2dc5cb4518ea03d18b4087ddf4ff`. CI should fetch that revision and write those five `git show` blobs into an owned temporary directory, preserving bytes. Original source trees are not bundled here. Each row explicitly maps its original path to `active_path`: the engine and two GDC variants are used only by canonical1.5/1.6. The three original shared assets must still match their original pins. Separate `html_routes` records verify that reversing exactly three script src substitutions restores each original HTML byte count/hash; no extra git extraction is needed.
