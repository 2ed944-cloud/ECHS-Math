...
require(html,['Private Bank Manager','Course Release Manager','zipFile','upload-manager.js','data-require-account="teacher admin"'],'Upload manager page')
require(js,['crypto.subtle.digest','signed_url','XMLHttpRequest','/complete','requireAuth(["teacher","admin"])'],'Upload manager client')
...
require(workflow,['schedule:','*/5 * * * *','SUPABASE_SERVICE_ROLE_KEY','process_teacher_upload_request.py','gh pr create'],'Upload processor workflow')
require(processor,['SHA-256 mismatch','safe_extract','upload_private_bank_package.py','ap-precalculus-unit-','github_pr_url'],'Upload processor')
require(deploy,['upload-manager-api/health'],'Backend deployment health check')
...
result=subprocess.run(['node','--check',str(ROOT/'question-bank/official/admin/js/upload-manager.js')],capture_output=True,text=True)
if result.returncode:errors.append(f'JavaScript syntax failure: {result.stderr}')
