...
require(js,['crypto.subtle.digest','signed_url','XMLHttpRequest','/complete','requireAuth(["teacher","admin"])','escapeHTML','bankCourseKey','target_courses','/requests/terminal','method:"DELETE"','Retrying failed package upload'],'Upload manager client')
require(css,['.dropZone','.progressBar','.requestItem','.historyDelete','.historyClear','@media'],'Upload manager styles')
require(api,['createSignedUploadUrl','teacher-upload-staging','api_session_lookup','Teacher or administrator sign-in is required','status: "queued"','"ap-calculus"','TERMINAL_STATUSES','Retry upload URL created','/requests/terminal','req.method === "DELETE"'],'Upload manager API')
