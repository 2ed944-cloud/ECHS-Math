...
require(api,['createSignedUploadUrl','teacher-upload-staging','CHUNK_BYTES=40*1024*1024','chunkMeta','part_paths','upload_mode:"chunked"','Upload part','1.3.0-free-tier-chunked','"ap-calculus"'],'Upload manager API')
...
require(workflow,['schedule:','*/5 * * * *','SUPABASE_SERVICE_ROLE_KEY','process_teacher_upload_request_fast.py','timeout-minutes: 120'],'Upload processor workflow')
require(fast_processor,['part_paths','Reassembled staged part','hashlib.sha256','base.download = download','upload_private_bank_package_verified_free_tier.py'],'Chunk reassembly processor')
...
for path in ['tools/process_teacher_upload_request.py','tools/process_teacher_upload_request_fast.py','tools/upload_private_bank_package_verified_fast.py','tools/upload_private_bank_package_verified_free_tier.py','tools/test_private_bank_source_archive_chunking.py','tools/validate_upload_manager.py']:
...
for path in ['question-bank/official/admin/js/upload-manager.js','question-bank/official/admin/js/private-bank-center.js','question-bank/js/private-bank-practice.js']:
