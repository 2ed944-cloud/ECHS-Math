...
async function shot(page,name){
  ...
  if(name.startsWith('student-'))for(const [id,target] of [['heroMastery','97%'],['heroToday','4'],['heroStreak','2']])assert.equal(await page.locator('#'+id).textContent(),target,name+': final reported metric '+id);
  ...
  metrics.meters=await page.locator('.meterCard').evaluateAll(cards=>cards.map(card=>{const copy=card.lastElementChild,h=copy.querySelector('h3'),r=card.getBoundingClientRect(),c=copy.getBoundingClientRect();return {title:h?.textContent,cardWidth:r.width,copyWidth:c.width,copyScrollWidth:copy.scrollWidth,copyClientWidth:copy.clientWidth,headingScrollWidth:h?.scrollWidth,headingClientWidth:h?.clientWidth,cardRight:r.right,copyRight:c.right};}));
  metrics.quickLaunch=await page.locator('.quickLaunchGrid').evaluateAll(grids=>grids.map(g=>({columns:getComputedStyle(g).gridTemplateColumns,width:g.getBoundingClientRect().width,items:[...g.children].map(n=>({width:n.getBoundingClientRect().width,scrollWidth:n.scrollWidth,clientWidth:n.clientWidth}))})));
  layoutMetrics.push(metrics);
  for(const meter of metrics.meters){assert.ok(meter.copyScrollWidth<=meter.copyClientWidth+1,name+': meter copy remains inside its card: '+meter.title);assert.ok(meter.headingScrollWidth<=meter.headingClientWidth+1,name+': meter heading remains readable: '+meter.title);}
  for(const grid of metrics.quickLaunch){assert.equal(grid.columns.trim().split(/\s+/).length,mobile?1:4,name+': intended quick-launch columns');for(const item of grid.items)assert.ok(item.scrollWidth<=item.clientWidth+1,name+': quick-launch content does not overflow');}
  if(mobile){
...
try{
  ...
  for(const mobile of [false,true]){
    ...
    await test('Teacher cloud, actual heatmap and Smart Route '+device+' keep scores provisional',async()=>{mode='high';const{ctx,page}=await open('/question-bank/teacher.html','teacher',{mobile});try{
      await page.waitForFunction(()=>document.getElementById('studentRows')?.textContent.includes('Strong practice'));await page.waitForFunction(()=>document.getElementById('classHeatmap')?.textContent.includes('Provisional practice'));assert.doesNotMatch(await page.locator('#studentRows').textContent(),/Mastered/);assert.equal(await page.locator('#classHeatmap .mastered').count(),0);assert.match(await page.locator('#attentionList').textContent(),/Insufficient practice evidence · 0 open mistakes/);assert.doesNotMatch(await page.locator('#attentionList').textContent(),/null%|undefined%/);assert.equal(await page.locator('#studentRows .masteryCell .progressMini i').nth(1).evaluate(n=>n.getBoundingClientRect().width),0);assert.equal(await page.locator('#classHeatmap .evidenceCell strong').textContent(),'97');await page.locator('#smartRoute-teacher .slrLane').first().waitFor();assert.deepEqual(await page.locator('#smartRoute-teacher .slrLaneCount').allTextContents(),['1','0','1']);await shot(page,'teacher-'+device);
      mode='zero';await page.locator('#classSelector').dispatchEvent('change');await page.waitForFunction(()=>document.querySelector('#studentRows .masteryCell strong')?.textContent.includes('0%'));assert.match(await page.locator('#attentionList').textContent(),/0% provisional practice · 0 open mistakes/);assert.match(await page.locator('#attentionList').textContent(),/Insufficient practice evidence · 0 open mistakes/);assert.equal(await page.locator('#studentRows .masteryCell strong').nth(1).locator('span').last().textContent(),'—');for(const fill of await page.locator('#studentRows .masteryCell .progressMini i').all())assert.equal(await fill.evaluate(n=>n.getBoundingClientRect().width),0);
    }finally{await ctx.close();}});
  ...
  await test('Updated worker purges old reporting HTML and never uses a cross-release offline label',async()=>{mode='high';const{ctx,page}=await open('/__c02/cache-prime.html','student',{workers:true});try{
    const stalePaths=['/question-bank/student.html','/question-bank/teacher.html','/question-bank/parent.html','/question-bank/dashboard.html','/index.html','/preview.html','/question-bank/js/learning-system.js','/question-bank/js/dashboard.js','/js/portal.js','/js/institution-experience.js'];
