...
def exercise(db,connect,passed):
 ...
 with db.transaction(force_rollback=True):
  ...
  rejected('Missing current companion fails closed for ordinary read committed writes',lambda:insert('learning_sessions',values('learning_sessions',target)),'40001')
 ...
 with ThreadPoolExecutor(max_workers=2) as pool:
  ...
  with connect() as deleter,connect() as legacy:
   ...
   while time.monotonic()<end:
    ...
    if future.done():assert future.result()=='40001';observed=True;break
    time.sleep(.02)
   assert observed,'Concurrent parent-delete overlap was not observed'
   deleter.execute('commit');assert future.result(timeout=6) in ('23503','40001')
  assert count('learning_sessions',target)==0
