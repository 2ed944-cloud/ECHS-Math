...
def validate_source(root: Path, errors: list[str]) -> None:
    ...
    require(private_base, ("IB Mathematics AI Bank 1", "IB Mathematics AI Bank 10", "row?.course_mappings", 'activeCourse==="ib-math-ai"', "IB Math AI · Uploaded Banks", "checking uploaded banks", "echs:bundle-progress", "expectedUnit!==mappedUnit", "private_bank_only", "private-upload-manager", "AP Calculus fallback banks were intentionally blocked", "return mergeUnique(activeCourse,[result.questions])"), "IB private-bank course browser", errors)
    ...
    require(worker, ("ibmapped1", "./css/ib-lesson-platform-integration.css", "./js/ib-lesson-platform-integration.js", "./question-bank/js/practice-global-bridge.js", "./question-bank/js/private-bank-practice.js", "./question-bank/js/ib-private-bank-lesson-aliases.js", "ib-private-bank-lesson-aliases", "question-bank\\/js\\/(?:bank|practice|"), "Service worker", errors)
