...
await group('exact fifteen routes and site origin; query/hash do not select another lesson',async()=>{
  assert.equal(INVESTIGATION_HOSTS.length,15);
...
await group('fifteen allowed hosts preserve original DOM and URL and import only on open',async()=>{
