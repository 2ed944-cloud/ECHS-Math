...
const teacher={id:'00000000-0000-4000-8000-000000000005',organization_id:'00000000-0000-4000-8000-000000000002',status:'active',expires_at:new Date(Date.now()+3600000).toISOString(),display_name:'Mohammad Abu Ghuwaleh',username:'m.abughuwaleh',role:'teacher',organization_name:'ECHS Mathematics'};
...
for(const device of devices){
  ...
  await page.addInitScript(account=>{localStorage.setItem('echs_institution_token_v1','upload-manager-visual-token');localStorage.setItem('echs_institution_account_v1',JSON.stringify(account));localStorage.setItem('echs_institution_expires_v1',account.expires_at)},teacher);
