"""Derived private fixture controls; one closed missing-journal-owner setup variant."""
import hashlib
import json
import ipaddress
import os
...
sys.path.insert(0,str(Path(__file__).resolve().parent.parent / "private-learning-browser-journal-http"))
...
def main(config_path):
    ...
    def token(account):
        ...
        return raw
    def new_case(role, journal_owner=True):
    ...
    try:
        for line in sys.stdin:
            ...
            try:
                ...
                if action == "new_missing_owner":
                    need(set(request) == {"id", "action"}, "control-shape")
                    result = new_case("student", journal_owner=False)
                elif action == "new":
