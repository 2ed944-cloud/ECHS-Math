...
const canonical={
  id:'g11-ib-ai',
  grade:'G11',
  title:'G11 IB Mathematics: Applications and Interpretation',
  shortTitle:'IB Math AI',
  units:Array.from({length:6},(_,index)=>({
    title:`Unit ${index+1}: Existing`,
    lessons:index===0
      ?Array.from({length:5},(_,lesson)=>({number:`1.${lesson+1}`,title:'Old'}))
      :Array.from({length:5},(_,lesson)=>({number:`${index+1}.${lesson+1}`,title:'Existing'}))
  }))
};
const duplicate={
  id:'ib-math-ai',
  grade:'IB DP',
  title:'IB Mathematics: Applications and Interpretation',
  shortTitle:'IB Math AI',
  units:[{title:'Unit 1: Duplicate',lessons:Array.from({length:8},(_,index)=>({number:`1.${index+1}`,title:'Duplicate'}))}]
};
const events=[];
...
const summary='6 lessons · 434 purposeful Learn screens · 556 studio questions · 86 quiz questions · 29 extended tasks';
...
unit.lessons.forEach((lesson,index)=>{
  ...
  if(lesson.title!==title||lesson.release!==release||lesson.learnSlides!==slides||lesson.practiceQuestions!==practice||lesson.quizQuestions!==quiz||lesson.extendedTasks!==tasks){
    throw new Error(`Lesson ${lesson.number} metadata mismatch: ${JSON.stringify(lesson)}`);
  }
...
for(const required of ['negative and rational exponents','common bases','logarithms','logarithmic-scale']){
  if(!lesson15.outcomes.some(outcome=>outcome.toLowerCase().includes(required)))throw new Error(`Lesson 1.5 outcome missing ${required}`);
}
if(lesson15.organization_release!=='6.1.0')throw new Error('Lesson 1.5 organization release is missing');
if(lesson15.organization!=='one lesson with four internal teaching blocks')throw new Error('Lesson 1.5 organization description is missing');
if(!Array.isArray(lesson15.teachingBlocks)||lesson15.teachingBlocks.length!==4)throw new Error('Lesson 1.5 must expose four teaching blocks');
...
if(lesson15.teachingBlocks.some(block=>block.estimatedClassroomTime!=='60–75 minutes'))throw new Error('Lesson 1.5 teaching-block pacing metadata is incomplete');
...
const totals=unit.lessons.reduce((acc,lesson)=>{
  acc.slides+=lesson.learnSlides;
  acc.practice+=lesson.practiceQuestions;
  acc.quiz+=lesson.quizQuestions;
  acc.tasks+=lesson.extendedTasks;
  return acc;
},{slides:0,practice:0,quiz:0,tasks:0});
if(JSON.stringify(totals)!==JSON.stringify({slides:434,practice:556,quiz:86,tasks:29}))throw new Error(`Unit totals are inconsistent: ${JSON.stringify(totals)}`);
...
console.log('IB Unit 1 six-lesson portal update and definitive Lesson 1.5: PASS');
