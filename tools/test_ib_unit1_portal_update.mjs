...
const summary='6 lessons · 376 purposeful Learn screens · 340 learning and practice questions · 42 quiz questions · 67 written tasks';
...
if(!unit.architectureNotes.some(note=>note.includes('TI-Nspire CX / CX II')))throw new Error('Lesson 1.1 real TI-84 note is missing');
...
const expected=[
 ...
 ['Arithmetic Sequences and Series','7.0.0',47,12,0,14],
...
unit.lessons.forEach((lesson,index)=>{
  ...
  if(!lesson.resources.some(resource=>resource.label===(['1.2','1.4'].includes(lesson.number)?'Learning checks and IB-style written practice':`Practice Studio · ${practice} questions`)))throw new Error(`Lesson ${lesson.number} practice resource count is stale`);
  if(!lesson.resources.some(resource=>resource.label===(['1.2','1.4'].includes(lesson.number)?`IB-style written tasks · ${tasks}`:`IB-style assessment tasks · ${tasks}`)))throw new Error(`Lesson ${lesson.number} task resource count is stale`);
});

const lesson11=unit.lessons[0];
if(lesson11.defaultScope!=='IB SL Core'||lesson11.allContentAvailable!==false)throw new Error('Lesson 1.1 scope metadata is incomplete');
if(JSON.stringify(lesson11.scopeCounts)!==JSON.stringify({learn:{core:43,all:43},practice:{core:12,all:12},quiz:{core:0,all:0},tasks:{core:14,all:14}}))throw new Error(`Lesson 1.1 scope counts are incorrect: ${JSON.stringify(lesson11.scopeCounts)}`);
if(lesson11.calculator?.model!=='TI-Nspire CX / CX II'||lesson11.calculator?.mode!=='guided calculator practice'||lesson11.calculator?.externalDependency!==false)throw new Error('Lesson 1.1 TI-Nspire guidance metadata is incomplete');
if(lesson11.officialCoreSections?.map(s=>s.code).join(',')!=='SL 1.1,SL 1.6')throw new Error('Merged official sections are missing');
if(lesson11.assessment?.writtenParts!==46||lesson11.assessment?.writtenMarks!==82)throw new Error('Written assessment totals are incorrect');
...
if(lesson12.officialCoreSections?.map(s=>s.code).join(',')!=='SL 1.2')throw new Error('Arithmetic official scope is missing');
if(lesson12.assessment?.writtenParts!==49||lesson12.assessment?.writtenMarks!==100)throw new Error('Arithmetic written totals are stale');
if(lesson12.teachingBlocks?.map(b=>b.code).join(',')!=='1.2A,1.2B')throw new Error('Arithmetic teaching blocks are incomplete');
if(!lesson12.outcomes.some(o=>o.includes('simple interest')&&o.includes('approximate')))throw new Error('Arithmetic application outcomes are missing');
...
if(JSON.stringify(totals)!==JSON.stringify({slides:376,practice:340,quiz:42,tasks:67}))throw new Error(`Unit totals are inconsistent: ${JSON.stringify(totals)}`);
...
console.log('IB Unit 1 six-lesson portal update with Lesson 1.1 TI-Nspire CX / CX II release: PASS');
