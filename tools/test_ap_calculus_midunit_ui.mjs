...
assert.equal(document.documentElement.dataset.checkpointReady,'true');assert.equal(document.querySelectorAll('[data-question]').length,Q.questions.length);assert.equal(document.querySelectorAll('.slide:not([hidden])').length,1);assert.equal(document.querySelectorAll('svg').length,Q.questions.reduce((n,q)=>n+(q.graph?1:0)+(q.graphs?.length||0)+(q.choiceGraphs?.length||0),0)+(Q.frqs?.length||0));
...
// Written responses preserve drafts, gate rubrics, and separate self-assessment from MCQ scores.
const write=(id,text)=>{const n=$('draft-'+id);n.value=text;n.dispatchEvent(new f.dom.Event('input',{bubbles:true}));};
const selfScore=(id,value)=>{const n=$('self-score-'+id);n.value=value;n.dispatchEvent(new f.dom.Event('change',{bubbles:true}));};
const wid='frq01-a';f.goHash('#section=written-frq01');assert.equal(current(),'written-frq01');f.key($('draft-'+wid),'ArrowRight');assert.equal(current(),'written-frq01');
click(document.querySelector(`[data-rubric="${wid}"]`));assert.ok($('rubric-'+wid).hidden);assert.match($('written-feedback-'+wid).textContent,/Write your response/);assert.ok($('self-score-'+wid).disabled);
write(wid,'The graph gives 3; the table suggests 5, so the difference approaches -2. <script>plain text</script>');
assert.equal($('writtenDraftCount').textContent,'1 / 6');click(document.querySelector(`[data-rubric="${wid}"]`));assert.equal($('rubric-'+wid).hidden,false);assert.equal($('self-score-'+wid).disabled,false);selfScore(wid,'2');assert.equal($('writtenAssessedCount').textContent,'1 / 6');assert.equal($('correctCount').textContent,String(N-1));
const writtenReload=fixture();assert.match(writtenReload.$('draft-'+wid).value,/plain text/);assert.equal(writtenReload.$('self-score-'+wid).value,'2');assert.equal(writtenReload.document.querySelector('.slide:not([hidden])').id,'written-frq01');
write(wid,'Revised reasoning');assert.ok($('rubric-'+wid).hidden);assert.ok($('self-score-'+wid).disabled);assert.equal($('writtenAssessedCount').textContent,'0 / 6');click(document.querySelector(`[data-rubric="${wid}"]`));selfScore(wid,'0');assert.equal($('writtenAssessedCount').textContent,'1 / 6');
write('frq02-c','Nearest difference quotient gives -1.4 V/s.');click(document.querySelector('[data-written-flag="frq02"]'));assert.equal($('flaggedCount').textContent,'2');assert.equal($('writtenDraftCount').textContent,'2 / 6');
f.account({id:'student-b'});assert.equal($('draft-'+wid).value,'');assert.equal($('writtenDraftCount').textContent,'0 / 6');assert.equal($('writtenAssessedCount').textContent,'0 / 6');assert.equal($('correctCount').textContent,'0');assert.equal($('flaggedCount').textContent,'0');assert.ok($('hint-q01').hidden);assert.equal(current(),'start');
answer('q02',Q.questions[1].answer);click(document.querySelector('[data-check="q02"]'));assert.equal($('firstCount').textContent,'1');f.account({id:'student-a'});assert.equal($('draft-'+wid).value,'Revised reasoning');assert.equal($('self-score-'+wid).value,'0');assert.equal($('writtenAssessedCount').textContent,'1 / 6');assert.equal($('correctCount').textContent,String(N-1));
click($('resetWork'));assert.equal($('resetConfirmation').hidden,false);click($('cancelReset'));assert.equal($('correctCount').textContent,String(N-1));click($('resetWork'));click($('confirmReset'));assert.equal($('correctCount').textContent,'0');assert.equal($('flaggedCount').textContent,'0');assert.ok($('solution-q01').hidden);assert.equal($('draft-'+wid).value,'');assert.equal($('writtenDraftCount').textContent,'0 / 6');assert.equal($('writtenAssessedCount').textContent,'0 / 6');assert.ok($('self-score-'+wid).disabled);
...
console.log('AP Calculus checkpoint interactions: PASS (answers, hints, solutions, flags, scoring, navigation, append persistence, written drafts/rubrics/self-assessment, account isolation, reset, unavailable storage, protected practice routes)');
