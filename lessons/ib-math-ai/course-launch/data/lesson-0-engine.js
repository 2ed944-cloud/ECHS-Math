(()=>{"use strict";
const D=window.ECHS_IBAI_L0_DATA,Q=window.ECHS_IBAI_L0_DIAGNOSTIC;
if(!D||!Q)throw new Error("IB Math AI Lesson 0 data failed to load.");
...
const state={
 track:localStorage.getItem(STORE+":track")||"SL",
...
function updateTrackUI(){
 ...
 const sel=$("#identityTrack");if(sel)sel.value=state.track==="HL"?"IB Math AI HL":"IB Math AI SL";
 renderStrand("G");renderSubmitSummary();renderHLResult();
}
$$(".track-btn[data-track]").forEach(b=>b.onclick=()=>{state.track=b.dataset.track;save();updateTrackUI();say(`${state.track} level selected`)});
...
$("#identityTrack")?.addEventListener("change",e=>{state.track=e.target.value.includes("HL")?"HL":"SL";save();updateTrackUI()});
...
function renderStrand(strand){
 ...
 if(strand==="G"&&state.track!=="HL"){host.innerHTML='<div class="notice">HL extension is hidden because SL is selected. Switch to HL on the Level slide to activate it.</div>';return}
...
function requiredQuestions(){return Q.questions.filter(q=>commonStrands.includes(q.strand)||(state.track==="HL"&&q.strand==="G"))}
function renderSubmitSummary(){
 ...
 host.innerHTML=`<strong>${answered} / ${req.length} answered.</strong> ${state.track==="HL"?"HL includes the 6-item extension.":"SL requires the 36 common items."}`;
}
renderSubmitSummary();
function band(p){if(p>=85)return["Ready","Your prerequisites are broadly secure. Maintain them through cumulative retrieval."];if(p>=70)return["Ready with targeted repair","Begin the IB course while repairing the weakest one or two strands."];if(p>=55)return["Priority repair","You can begin the course, but prerequisite repair should be scheduled immediately."];return["Intensive prerequisite support","Multiple prerequisite strands need structured repair alongside Unit 1."]}
...
function renderHLResult(){
 const host=$("#hlResult");if(!host)return;
 if(!submitted){host.innerHTML='<div class="notice">HL extension results appear after submission.</div>';return}
 if(state.track!=="HL"){host.innerHTML='<div class="notice">SL selected. HL extension is not part of your readiness profile.</div>';return}
 const r=strandResults("G");host.innerHTML=`<div class="result-card"><h3>HL extension · ${r.correct}/6 (${r.pct}%)</h3><p>${r.pct>=80?"Strong readiness for HL-specific prerequisite notation and structures.":r.pct>=60?"Some targeted HL prerequisite review is recommended.":"Prioritize matrices, vectors, series, advanced probability and symbolic structure before HL-only work."}</p></div>`
}
...
function renderAllResults(){renderOverall();renderProfile();renderHLResult();renderPriorities();renderMissed();renderConfidence();renderFirstWeek();markDiagnostics();typeset(document)}
...
function exportSummary(){
 ...
 const r=commonResults(),lines=[`IB Math AI Lesson 0 Diagnostic`,`Level: ${state.track}`,`Common readiness: ${r.correct}/36 (${r.pct}%)`,""];
 commonStrands.forEach(s=>{const x=strandResults(s);lines.push(`${s} ${Q.strands[s].name}: ${x.correct}/6 (${x.pct}%)`)});
 if(state.track==="HL"){const x=strandResults("G");lines.push(`HL Extension: ${x.correct}/6 (${x.pct}%)`)}
 ...
 const blob=new Blob([lines.join("\n")],{type:"text/plain"}),a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download="IB_Math_AI_Lesson_0_Diagnostic_Summary.txt";a.click();URL.revokeObjectURL(a.href)
}
