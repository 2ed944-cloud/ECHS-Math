(function(){
  ...
  const concept=(section,title,eyebrow,lead,items,visual='',note='')=>S(section,title,'content',eyebrow,`<div class="pr5-concept ${visual?'with-visual':''}"><div>${lead?`<p class="pr5-lead">${lead}</p>`:''}${cards(items)}${note?`<div class="pr5-note">${note}</div>`:''}</div>${visual?V(visual,title):''}</div>`);
  const W=(section,title,prompt,steps,result,interpretation='',visual='')=>S(section,title,'worked','Worked example',`<div class="pr5-worked"><section class="pr5-question"><span>QUESTION</span><h2>${prompt}</h2>${visual?V(visual,title):''}</section><section class="pr5-method"><span>METHOD</span><ol>${steps.map(step=>`<li>${step}</li>`).join('')}</ol><div class="pr5-result">${result}</div>${interpretation?`<p class="pr5-interpret">${interpretation}</p>`:''}</section></div>`);
