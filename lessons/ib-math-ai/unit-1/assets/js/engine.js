...
function renderPractice() {
  ...
  app.innerHTML = `<section class="route-page">
    <header class="route-header"><span class="slide-section">Practice Studio</span><h1>Build fluency, modelling and reasoning</h1><p>Forty original questions are balanced across four levels. Work is saved locally on this device.</p></header>
    <div class="studio-toolbar">${levels.map(level => `<button class="filter-btn ${level === practiceFilter ? 'active' : ''}" data-filter="${level}">${level}${level === 'All' ? ' · 40' : ' · 10'}</button>`).join('')}</div>
...
function renderExam() {
  showFooter(false);
  app.innerHTML = `<section class="route-page"><header class="route-header"><span class="slide-section">IB-style assessment</span><h1>Extended-response practice</h1><p>Two original tasks per lesson. Use command terms, show technology transparently and interpret every contextual result.</p></header>
...
function initializeLab() {
  ...
  const configs = {
    '1.1': {fields:[['coefficient','Coefficient',4.8],['exponent','Exponent',7],['multiplier','Multiplier',2.5],['multiplierExponent','Multiplier exponent',-3]], calculate(v){const value=v.coefficient*10**v.exponent*v.multiplier*10**v.multiplierExponent;return `Product = ${formatNumber(value)} = ${value.toExponential(5)}`;}},
    '1.2': {fields:[['first','First term',28],['difference','Common difference',3],['terms','Number of terms',24]], calculate(v){const n=Math.max(1,Math.round(v.terms));const last=v.first+(n-1)*v.difference;const sum=n*(v.first+last)/2;return `u_${n} = ${formatNumber(last)}; S_${n} = ${formatNumber(sum)}`;}},
    '1.3': {fields:[['first','First term',600],['ratio','Common ratio',1.12],['terms','Number of terms',8]], calculate(v){const n=Math.max(1,Math.round(v.terms));const term=v.first*v.ratio**(n-1);const sum=Math.abs(v.ratio-1)<1e-12?v.first*n:v.first*(1-v.ratio**n)/(1-v.ratio);return `u_${n} = ${formatNumber(term)}; S_${n} = ${formatNumber(sum)}`;}},
    '1.4': {fields:[['principal','Principal (QAR)',18000],['rate','Annual rate (%)',4.6],['years','Years',5],['compounds','Compounds per year',1]], calculate(v){const m=Math.max(1,Math.round(v.compounds));const amount=v.principal*(1+v.rate/100/m)**(m*v.years);return `Future value = QAR ${formatNumber(amount,2)}; growth = QAR ${formatNumber(amount-v.principal,2)}`;}},
    '1.5': {fields:[['base','Base',10],['argument','Positive argument',3162.27766],['initial','Initial value',650],['target','Target value',3000]], calculate(v){if(v.base<=0||v.base===1||v.argument<=0||v.initial<=0||v.target<=0)return 'Use a positive base other than 1 and positive arguments.';const log=Math.log(v.argument)/Math.log(v.base);const t=Math.log(v.target/v.initial)/Math.log(1.14);return `log_base(argument) = ${formatNumber(log)}; time for 650(1.14)^t = target: t = ${formatNumber(t)} hours`;}},
    '1.6': {fields:[['reported','Reported value',84],['unit','Rounding unit',1],['second','Second reported value',57],['secondUnit','Second rounding unit',1]], calculate(v){const low1=v.reported-v.unit/2, high1=v.reported+v.unit/2, low2=v.second-v.secondUnit/2, high2=v.second+v.secondUnit/2;return `First interval [${formatNumber(low1)}, ${formatNumber(high1)}); product interval [${formatNumber(low1*low2)}, ${formatNumber(high1*high2)})`;}},
    '1.7': {fields:[['principal','Loan principal',85000],['rate','Nominal annual rate (%)',5.2],['years','Years',6],['payments','Payments per year',12]], calculate(v){const m=Math.max(1,Math.round(v.payments));const n=Math.max(1,Math.round(v.years*m));const i=v.rate/100/m;const payment=i===0?v.principal/n:v.principal*i/(1-(1+i)**(-n));return `Periodic payment = QAR ${formatNumber(payment,2)}; total interest = QAR ${formatNumber(payment*n-v.principal,2)}`;}},
    '1.8': {fields:[['a','a in ax+by=c',2],['b','b',3],['c','c',17],['d','d in dx+ey=f',5],['e','e',-2],['f','f',4]], calculate(v){const det=v.a*v.e-v.b*v.d;if(Math.abs(det)<1e-12)return 'The determinant is zero: the system has no unique solution.';const x=(v.c*v.e-v.b*v.f)/det;const y=(v.a*v.f-v.c*v.d)/det;return `Unique solution: x = ${formatNumber(x)}, y = ${formatNumber(y)}; determinant = ${formatNumber(det)}`;}}
  };
  const config = configs[number];
  if (!config) return;
  lab.innerHTML = `<div class="lab-grid">${config.fields.map(([key,label,value]) => `<label>${label}<input type="number" step="any" data-lab-field="${key}" value="${value}"></label>`).join('')}</div><button class="primary-btn" id="run-lab">Calculate and interpret</button><div class="lab-output" id="lab-output">Change the inputs, predict, then calculate.</div>`;
  const run = () => {
    const values = Object.fromEntries($$('[data-lab-field]', lab).map(input => [input.dataset.labField, Number(input.value)]));
    $('#lab-output', lab).textContent = config.calculate(values);
  };
  $('#run-lab', lab).addEventListener('click', run);
  $$('[data-lab-field]', lab).forEach(input => input.addEventListener('change', run));
}
