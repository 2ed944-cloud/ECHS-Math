...
(function(){
  ...
  function go(index,focus=true){current=Math.max(0,Math.min(slides.length-1,Number(index)||0));for(const [i,s] of slides.entries()){s.hidden=i!==current;s.classList.toggle('active',i===current);}const s=slides[current];state.slide=s.id;$('slideSelect').value=String(current);$('topCounter').textContent=`${current+1} of ${slides.length}`;$('slideCounter').textContent=`${current+1} / ${slides.length}`;$('progressBar').style.width=`${(current+1)/slides.length*100}%`;$('previousSlide').disabled=current===0;$('nextSlide').disabled=current===slides.length-1;s.scrollTop=0;try{history.replaceState(null,'',location.pathname+location.search+'#'+(s.dataset.question?'question='+s.dataset.question:'section='+s.id));}catch{}if(focus)s.querySelector('h1,h2')?.focus({preventScroll:true});$('slideAnnouncement').textContent=`${current+1} of ${slides.length}: ${s.dataset.title}`;save();}
  const goId=id=>go(slides.findIndex(s=>s.id===id));
  function hashIndex(){const q=location.hash.match(/^#question=([\w-]+)$/);if(q)return slides.findIndex(s=>s.dataset.question===q[1]);const section=location.hash.match(/^#section=([\w-]+)$/);if(section)return slides.findIndex(s=>s.id===section[1]);const n=location.hash.match(/^#slide=(\d+)$/);if(n){const originalEnd={27:'review',28:'reference'}[Number(n[1])];return originalEnd?slides.findIndex(s=>s.id===originalEnd):Number(n[1])-1;}return slides.findIndex(s=>s.id===state.slide);}
