...
(function(root){
  ...
  function svg(g){
    ...
    const curves=g.branches.map(b=>{const points=b.points||Array.from({length:161},(_,i)=>{const x=b.domain[0]+i*(b.domain[1]-b.domain[0])/160;return[x,evaluate(b.coefficients,x)];});return `<polyline points="${points.map(([x,y])=>`${X(x).toFixed(3)},${Y(y).toFixed(3)}`).join(' ')}" fill="none" stroke="${b.kind==='tangent'?'#861e42':'#1269a0'}" stroke-width="3.2"${b.kind==='tangent'?' stroke-dasharray="8 5"':''}/>`;}).join('');
