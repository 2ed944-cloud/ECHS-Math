...
window.ECHSRatesLabs=function({M,G,esc,feedback,renderMath,mathHTML}){
  ...
  document.querySelectorAll('[data-rates-lab]').forEach(host=>{
    ...
    const defaults=[...host.querySelectorAll('[data-setting]')].map(el=>[el,el.value]);
    const reset=document.createElement('button');reset.type='button';reset.className='btn outline';reset.textContent='Reset investigation';reset.id=id+'-reset';host.querySelector('.actions').append(reset);reset.addEventListener('click',()=>{defaults.forEach(([el,value])=>el.value=value);update();});
