(async()=>{
  ...
  const dashboardAction=document.getElementById("libraryDashboardAction");
  const libraryLinks=[...document.querySelectorAll("[data-teacher-library]")];
  if(!entry&&!tile&&!dashboardAction&&!libraryLinks.length)return;
  ...
  if(current){
    ...
    if(dashboardAction)dashboardAction.href=href;
    ...
    const loginHref=ECHSInstitution.root("login.html");
    document.body.dataset.signedIn="false";
    delete document.body.dataset.accessRole;
    if(entry){entry.href=loginHref;entry.textContent="Sign in";entry.classList.remove("institutionSignedIn")}
    if(dashboardAction)dashboardAction.href=loginHref;
    updateTile("School account","Secure role-based access",cfg.enabled?"Students, teachers, families and administrators enter through school-created accounts.":"The secure institutional account system is prepared and awaiting backend activation.",loginHref,"Sign in to my account →");
