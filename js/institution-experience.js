...
(function(){
  ...
  function bindReadinessEntry(){
    $$(".institutionNav").forEach(nav=>{
      if(nav.querySelector("[data-readiness-entry]"))return;
      const link=document.createElement("a");
      link.href=new URL("readiness/index.html",ROOT).href;
      link.dataset.readinessEntry="true";
      link.innerHTML='<span class="institutionNavIcon">∑</span>IB Readiness';
      const sections=$$(".institutionNavSection",nav);
      const platformSection=sections.find(section=>/platform/i.test(section.textContent||""));
      if(platformSection)nav.insertBefore(link,platformSection);else nav.append(link);
    });
  }
  function loadCompletionLayer(){if(!document.querySelector('link[data-premium-responsive]')){const responsive=document.createElement("link");responsive.rel="stylesheet";responsive.href=new URL("css/institution-responsive.css?v=20260726-final",ROOT).href;responsive.dataset.premiumResponsive="true";document.head.append(responsive)}if(!document.querySelector('link[data-premium-completion]')){const link=document.createElement("link");link.rel="stylesheet";link.href=new URL("css/institution-completion.css?v=20260726-complete",ROOT).href;link.dataset.premiumCompletion="true";document.head.append(link)}if(!document.querySelector('script[data-premium-completion]')){const completion=document.createElement("script");completion.src=new URL("js/institution-completion.js?v=20260726-complete",ROOT).href;completion.dataset.premiumCompletion="true";completion.defer=true;document.head.append(completion)}}
  function loadGamification(){if(document.body.dataset.premiumPage!=="student")return;if(!document.querySelector('link[data-echs-gamification]')){const link=document.createElement("link");link.rel="stylesheet";link.href=new URL("css/gamification.css?v=20260727-pathway",ROOT).href;link.dataset.echsGamification="true";document.head.append(link)}if(!document.querySelector('script[data-echs-gamification]')){const scriptNode=document.createElement("script");scriptNode.src=new URL("js/gamification-overlay.js?v=20260727-pathway",ROOT).href;scriptNode.dataset.echsGamification="true";scriptNode.defer=true;document.head.append(scriptNode)}}
  function loadMasteryEvidence(){if(!document.querySelector('link[data-mastery-evidence]')){const link=document.createElement("link");link.rel="stylesheet";link.href=new URL("css/mastery-evidence.css?v=20260727-foundation1",ROOT).href;link.dataset.masteryEvidence="true";document.head.append(link)}if(!document.querySelector('script[data-mastery-evidence-client]')){const client=document.createElement("script");client.src=new URL("js/institution-mastery-evidence.js?v=20260727-foundation1",ROOT).href;client.dataset.masteryEvidenceClient="true";client.defer=true;document.head.append(client)}if(document.body.dataset.premiumPage==="teacher"&&!document.querySelector('script[data-teacher-evidence]')){const teacher=document.createElement("script");teacher.src=new URL("question-bank/js/teacher-evidence-heatmap.js?v=20260727-foundation1",ROOT).href;teacher.dataset.teacherEvidence="true";teacher.defer=true;document.head.append(teacher)}}
  function init(){bindTheme();bindDate();bindAnimatedNumbers();bindDialogFocusRecovery();bindGuideShortcutCompatibility();bindReadinessEntry();const current=window.ECHSInstitution?.account?.();if(current)setIdentityAvatar(current);loadCompletionLayer();loadGamification();loadMasteryEvidence()}
  if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",init,{once:true});else init();
  window.ECHSExperience={ROOT:ROOT.href,clamp,escapeHTML,icon,greeting,formatDate,safePercent,setRing,setIdentityAvatar,showPreview,renderWeekBars,renderDistribution,toast,bindReadinessEntry,loadCompletionLayer,loadGamification,loadMasteryEvidence};
})();
