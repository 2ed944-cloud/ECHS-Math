# ECHS Lead Scout Queue — 2026-08-27

## Sending gate

**HOLD — NO OUTBOUND SEND IN THIS RUN.**

- Gmail rolling 24-hour SENT count at review time: **38 messages**.
- Recent delivery health remains elevated-risk, with multiple hard-bounce signals including nonexistent recipients/domains and MX/DNS failure.
- Before any next wave, rerun both the rolling-24h sent-volume check and current failure/bounce check.
- The current run therefore performs research, qualification, duplicate suppression, and queue retention only.

## Absolute exclusions and coordination rules
...
The AP Precalculus materials in the current ECHS repository provide a controlled high-rigor lesson/practice/assessment workflow with AP-style questions, differentiated Practice Studio routes, calculator/no-calculator labeling, worked solutions, student-turn cycles, timed assessment, and interactive explorations. Future school outreach should describe these as ECHS product capabilities only, never as a prior customer deployment or case study.

### IB Mathematics AI SL

The repository-validated IB Mathematics: Applications and Interpretation SL Unit 1 release contains eight lessons. The current release records, per lesson, 61 Learn slides, 52 Practice Studio questions, 14 timed-quiz questions, and 3 extended IB-style tasks; the release was merged to `main` on 2026-08-02. The QA report also records a remaining signed-in production smoke-test step, so outreach must not overstate operational deployment evidence.
...
- Verified institutional fallback route exists (`reception@asd.edu.qa`), but a direct current decision-maker address should be verified before sending because named academic routing is preferred.
...
**Why qualified**
- Current IB organization data shows students registered in **Mathematics: Applications and Interpretation** and Mathematics: Analysis and Approaches.
- Current school materials state a full-IB direction; MYP and CP authorizations in 2025 are strong curriculum-expansion signals.
- Current school figures indicate **1,250+ students**.

**Named academic target**
- Mr. John Bastonero Labor — Middle & High School Vice Principal / IBDP Coordinator.
- Verified institutional route: `info@apgschool.com`.
- Do not guess a direct personal email; verify one before sending if available.
...
**Why conditional**
- Current program includes AP Calculus AB and a Precalculus/AP Calculus pathway.
- Named secondary leadership is publicly available.
- However, it is a DoDEA/public-school environment rather than the preferred private/international commercial segment; procurement complexity lowers priority.
...
- Current public evidence reviewed in this run did **not** establish a sufficiently strong, current AP-math or Math AI/Applications-specific match for the ECHS pitch.
- Leadership data also requires reconciliation before outreach.
