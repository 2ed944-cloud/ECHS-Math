(function () {
  ...
  if (!Array.isArray(window.ECHS_COURSES)) {
    window.ECHS_COURSES = [];
  }
  ...
  "lessons": [
    ...
    },
    {
      ...
      "resources": [
        {
          "label": "Interactive classroom lesson · complete graph, rate and covariation coverage",
  ...
  var courseIndex = window.ECHS_COURSES.findIndex(function (item) {
    ...
    if (id === "ap-precalculus-g10-g11" || id === "ap-precalculus") {
      return true;
    }

    return [item.course, item.title, item.shortTitle].some(function (label) {
      var text = normalize(label);
      return text === "g10-g11 ap precalculus" ||
             text === "ap precalculus" ||
             (text.indexOf("ap precalculus") >= 0 && text.indexOf("g10") >= 0);
    });
  });

  if (courseIndex >= 0) {
    var targetCourse = window.ECHS_COURSES[courseIndex];

    targetCourse.id = targetCourse.id || "ap-precalculus-g10-g11";
    targetCourse.course = targetCourse.course || "G10–G11 AP Precalculus";
    targetCourse.title = targetCourse.title || "G10–G11 AP Precalculus";
    targetCourse.shortTitle = targetCourse.shortTitle || "AP Precalculus";
    targetCourse.grade = targetCourse.grade || "G10–G11";

    if (!Array.isArray(targetCourse.units)) {
      targetCourse.units = [];
    }
    ...
    window.ECHS_COURSES.push({
      id: "ap-precalculus-g10-g11",
      grade: "G10–G11",
      title: "G10–G11 AP Precalculus",
